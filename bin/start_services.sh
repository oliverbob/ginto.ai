...
if [ ! -f "$SDCPU_PID_FILE" ]; then
  if [ -d "tools/sdcpu/venv" ] && [ -f "tools/sdcpu/src/api_server.py" ]; then
    ...
    # Fix log file permissions if owned by different user
    [ -f /tmp/sdcpu.log ] && rm -f /tmp/sdcpu.log 2>/dev/null || true
    cd tools/sdcpu
...
# === Ginto Tunnel Server ===
# Only restart if systemd service exists (production server)
if systemctl list-units --type=service --all 2>/dev/null | grep -q ginto-tunnel; then
  echo "Restarting Ginto Tunnel service..."
  sudo systemctl restart ginto-tunnel 2>/dev/null && echo "Tunnel service restarted" || echo "Warning: Failed to restart tunnel service"
elif [ -f "tools/tunnel/tunnel_server.php" ]; then
  # Dev mode: run tunnel server in background
  TUNNEL_PID_FILE="/tmp/ginto-tunnel.pid"
  if [ -f "$TUNNEL_PID_FILE" ]; then
    TPID=$(cat "$TUNNEL_PID_FILE")
    if ps -p "$TPID" > /dev/null 2>&1; then
      echo "Tunnel server already running (PID: $TPID)"
    else
  ...
  if [ ! -f "$TUNNEL_PID_FILE" ]; then
    echo "Starting Tunnel server on port 8765..."
    nohup php tools/tunnel/tunnel_server.php > /tmp/ginto-tunnel.log 2>&1 &
    echo $! > "$TUNNEL_PID_FILE"
    echo "Tunnel server started (pid $(cat $TUNNEL_PID_FILE), log: /tmp/ginto-tunnel.log)"
  fi
fi
