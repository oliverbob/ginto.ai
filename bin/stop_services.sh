...
# Stop SDCPU (FastSD CPU Image Generation Server)
SDCPU_PID_FILE="/tmp/ginto-sdcpu.pid"
if [ -f "$SDCPU_PID_FILE" ]; then
  PID=$(cat "$SDCPU_PID_FILE")
  if ps -p "$PID" > /dev/null 2>&1; then
    kill "$PID" && echo "Stopped SDCPU server (pid $PID)"
    sleep 1
    if ps -p "$PID" > /dev/null 2>&1; then
      kill -9 "$PID" || true
    fi
  fi
  rm -f "$SDCPU_PID_FILE" || true
else
  echo "No SDCPU pid file; nothing to stop."
fi

# Stop Tunnel server (dev mode only - systemd service manages production)
TUNNEL_PID_FILE="/tmp/ginto-tunnel.pid"
if [ -f "$TUNNEL_PID_FILE" ]; then
  PID=$(cat "$TUNNEL_PID_FILE")
  if ps -p "$PID" > /dev/null 2>&1; then
    kill "$PID" && echo "Stopped tunnel server (pid $PID)"
    sleep 1
    if ps -p "$PID" > /dev/null 2>&1; then
      kill -9 "$PID" || true
    fi
  ...
  echo "No tunnel pid file; nothing to stop."
fi
