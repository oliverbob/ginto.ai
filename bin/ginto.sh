...
#   network [mode]    Configure network mode (bridge/macvlan)
...
#
# Network Modes:
#   bridge   - Default LXD bridge, 10.0.0.0/8, 16.7M unique IPs
...
#   bin/ginto.sh network macvlan         # Enable full 32-bit IP space
#===============================================================================
...
readonly BASE_IMAGE="ginto-sandbox-image"
...
# Force mode: skip interactive prompts (auto-delete existing containers)
...
# Update installation status for web notification
update_install_status() {
    ...
    mkdir -p "$storage_dir" 2>/dev/null || sudo mkdir -p "$storage_dir"
    # Ensure directory is writable
    if [[ ! -w "$storage_dir" ]]; then
        sudo chown "$(whoami):$(whoami)" "$storage_dir" 2>/dev/null || true
    fi
    local status_file="${storage_dir}/.image_install_status"
    local content="{
    \"status\": \"$status\",
    \"step\": \"$step\",
    \"message\": \"$message\",
    ...
    echo "$content" > "$status_file" 2>/dev/null || echo "$content" | sudo tee "$status_file" > /dev/null
}

die() {
    log_error "$1"
    update_install_status "error" "failed" "$1" "false"
    exit 1
}
...
#
ensure_xfce_autostart() {
    local container="$1"
    
    # Check if xfce4 init script exists
    if ! $LXC_CMD exec "$container" -- test -f /etc/init.d/xfce4 2>/dev/null; then
        log_info "Installing XFCE autostart service in '$container'..."
        
        $LXC_CMD exec "$container" -- sh -c 'cat > /etc/init.d/xfce4 << '\''XFCEEOF'\''
#!/sbin/openrc-run
...
pidfile="/run/xfce4.pid"
start_stop_daemon_args="--wait 3000"
...
XFCEEOF'
        $LXC_CMD exec "$container" -- chmod +x /etc/init.d/xfce4
        $LXC_CMD exec "$container" -- rc-update add xfce4 default 2>/dev/null || true
    fi
    
    # Configure XFCE panels: top bar + bottom dock (if not already configured)
    if ! $LXC_CMD exec "$container" -- test -f /root/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml 2>/dev/null; then
        log_info "Configuring XFCE panels in '$container'..."
...
<channel name="xfce4-panel" version="1.0">
  ...
  <property name="panels" type="array">
    ...
    <property name="panel-2" type="empty">
      <property name="position" type="string" value="p=10;x=640;y=708"/>
...
PANELEOF
    ...
    $LXC_CMD exec "$container" -- rc-service xvfb start 2>/dev/null || true
    sleep 2
    $LXC_CMD exec "$container" -- rc-service x11vnc start 2>/dev/null || true
    sleep 1
    $LXC_CMD exec "$container" -- rc-service xfce4 start 2>/dev/null || true
}
...
cmd_init() {
    ...
    # Write initial status for web notification (reset any previous status)
    update_install_status "in_progress" "initializing" "Initializing LXD for Ginto sandboxes..." "false"
    ...
    if ! command -v lxc &>/dev/null && ! command -v /snap/bin/lxc &>/dev/null; then
        ...
        # Add www-data to lxd group for PHP to access LXD API socket
        sudo usermod -aG lxd www-data 2>/dev/null || true
        
        # Ensure snap bin is in PATH for this session
        export PATH="/snap/bin:$PATH"
    else
        log_success "LXD is already installed"
        # Ensure www-data is in lxd group for PHP LXD API access
        sudo usermod -aG lxd www-data 2>/dev/null || true
    fi
...
EOF
    ...
    # Check if template container exists (not image - we use lxc copy approach)
    if ! $LXC_CMD info "$BASE_CONTAINER" &>/dev/null; then
        log_warn "Template container '$BASE_CONTAINER' not found"
        
        # Clear stale install status marker since container doesn't exist
        local storage_dir="${PROJECT_ROOT}/../storage"
        local status_file="${storage_dir}/.image_install_status"
        if [[ -f "$status_file" ]]; then
            log_info "Clearing stale install status marker..."
            rm -f "$status_file"
        fi
        
        log_info "Creating template container from Alpine..."
        create_base_image
    else
        log_success "Template container '$BASE_CONTAINER' exists"
    fi
    ...
    # Get the absolute path to ginto.sh for sudoers
    local ginto_script_path="$(cd "$SCRIPT_DIR" && pwd)/ginto.sh"
    ...
    # Generate sudoers content for all web users
    {
        ...
        echo "# SECURITY NOTE:"
        echo "# - Web users can run lxc commands and ginto.sh without password"
        echo "# - Security is provided by LXC container isolation itself"
        echo "# - Each user gets their own isolated sandbox container"
        echo "#"
        ...
        for user in $lxc_users; do
            ...
            echo ""
            echo "# Allow lxc commands (container isolation provides security)"
            echo "$user ALL=(root) NOPASSWD: /snap/bin/lxc *"
            echo "$user ALL=(root) NOPASSWD: /snap/bin/lxc"
            echo ""
            echo "# Allow ginto.sh for sandbox management"
            echo "$user ALL=(root) NOPASSWD: /usr/bin/bash $ginto_script_path *"
            echo "$user ALL=(root) NOPASSWD: /bin/bash $ginto_script_path *"
            echo ""
        done
    ...
    # Install websockify on host for noVNC WebSocket proxy (if not already installed)
    if ! command -v websockify &>/dev/null; then
        log_info "Installing websockify for noVNC support..."
        if command -v apt-get &>/dev/null; then
            sudo apt-get install -y python3-websockify 2>/dev/null || \
            sudo pip3 install websockify --break-system-packages 2>/dev/null || \
            sudo pip3 install websockify 2>/dev/null || true
        elif command -v apk &>/dev/null; then
            sudo apk add --no-cache py3-websockify 2>/dev/null || \
            sudo pip3 install websockify 2>/dev/null || true
        fi
        if command -v websockify &>/dev/null; then
            log_success "websockify installed"
        else
            log_warn "Failed to install websockify - noVNC may not work"
        fi
    else
        log_success "websockify already installed"
    fi
    ...
    running_sandboxes=$($LXC_CMD list "${SANDBOX_PREFIX}" --format csv -c n,s 2>/dev/null | grep ",RUNNING" | cut -d',' -f1 || true)
    if [[ -n "$running_sandboxes" ]]; then
        log_info "Upgrading existing sandboxes with latest packages..."
        cmd_upgrade all
    fi
...
create_base_image() {
    ...
    if $LXC_CMD info "$container" &>/dev/null; then
        if [[ "$FORCE_MODE" == "true" ]]; then
            # Non-interactive mode: auto-delete existing container
            log_warn "Container '$container' already exists - deleting (force mode)"
            $LXC_CMD delete "$container" --force 2>/dev/null || true
        else
            echo ""
            log_warn "Container '$container' already exists."
            echo ""
            echo "  Options:"
            echo "    1) Delete existing container and start fresh"
            echo "    2) Resume/continue with existing container"
            echo ""
            read -p "  Choose option [1/2]: " choice
            
            case "$choice" in
                1)
                    log_info "Deleting existing container '$container'..."
                    $LXC_CMD delete "$container" --force 2>/dev/null || true
                    ;;
                2)
                    log_info "Resuming with existing container '$container'..."
                    # Skip container creation, jump to configuration
                    ;;
                *)
                    log_info "Invalid option. Resuming with existing container..."
                    ;;
            esac
        fi
    fi
    ...
    update_install_status "in_progress" "creating_base" "Creating ginto-sandbox base image..." "false"
    ...
    # Add alias to the locally cached Alpine image for easy identification
    # After lxc launch, the image is cached locally - get that fingerprint
    local alpine_fingerprint
    alpine_fingerprint=$($LXC_CMD image list --format csv -c f 2>/dev/null | head -1)
    if [ -n "$alpine_fingerprint" ]; then
        # Remove old alias if exists, then create new
        $LXC_CMD image alias delete "ginto-base" 2>/dev/null || true
        if $LXC_CMD image alias create "ginto-base" "$alpine_fingerprint" 2>/dev/null; then
            log_success "Added alias 'ginto-base' to Alpine base image ($alpine_fingerprint)"
        else
            log_warn "Failed to create ginto-base alias"
        fi
    else
        log_warn "Could not find local Alpine image fingerprint for alias"
    fi
    ...
    # Auto-detect latest PHP version available in Alpine
    local php_ver=$($LXC_CMD exec "$container" -- sh -c "apk search php8 | grep -oE 'php8[0-9]' | sort -ru | head -1")
    if [[ -z "$php_ver" ]]; then
        php_ver="php84"  # Fallback to PHP 8.4
        log_warn "Could not detect PHP version, using $php_ver"
    else
        log_info "Detected latest PHP version: $php_ver"
    fi
    
    log_info "Installing ${php_ver}..."
    update_install_status "in_progress" "installing_php" "Installing ${php_ver} and extensions..." "false"
    $LXC_CMD exec "$container" -- apk add --no-cache \
        ${php_ver} ${php_ver}-fpm ${php_ver}-mbstring ${php_ver}-openssl \
        ${php_ver}-pdo ${php_ver}-pdo_mysql ${php_ver}-pdo_sqlite ${php_ver}-curl \
        ${php_ver}-dom ${php_ver}-xml ${php_ver}-ctype ${php_ver}-session ${php_ver}-tokenizer \
        ${php_ver}-fileinfo ${php_ver}-iconv ${php_ver}-zip ${php_ver}-gd ${php_ver}-phar
    ...
    $LXC_CMD exec "$container" -- apk add --no-cache vim nano bash curl ca-certificates openrc iputils net-tools
    
    log_info "Installing document creation tools (pandoc, wkhtmltopdf, xvfb)..."
    $LXC_CMD exec "$container" -- apk add --no-cache pandoc wkhtmltopdf xvfb-run font-noto ttf-dejavu fontconfig || true
    $LXC_CMD exec "$container" -- fc-cache -f || true
    
    log_info "Installing VNC server and X11 for visual desktop..."
    $LXC_CMD exec "$container" -- apk add --no-cache \
        xvfb x11vnc xfce4 xfce4-terminal dbus dbus-x11 mesa-dri-gallium \
        font-noto-cjk ttf-freefont adwaita-icon-theme chromium || true
    
    # Create Chromium wrapper for root user (needs --no-sandbox)
    log_info "Configuring Chromium for root..."
    $LXC_CMD exec "$container" -- sh -c 'cat > /usr/local/bin/chromium-browser << '\''CHROMEOF'\''
#!/bin/sh
exec /usr/bin/chromium --no-sandbox --disable-gpu --disable-software-rasterizer --disable-dev-shm-usage "\$@"
CHROMEOF'
    $LXC_CMD exec "$container" -- chmod +x /usr/local/bin/chromium-browser
    $LXC_CMD exec "$container" -- sed -i "s|Exec=chromium|Exec=/usr/local/bin/chromium-browser|g" /usr/share/applications/chromium.desktop 2>/dev/null || true
    
    # Configure XFCE to use Chromium as default browser
    $LXC_CMD exec "$container" -- sh -c '
        sed -i "s|X-XFCE-Binaries=chromium;chromium-browser;|X-XFCE-Binaries=chromium-browser;|" /usr/share/xfce4/helpers/chromium.desktop 2>/dev/null || true
        sed -i "s|X-XFCE-Commands=%B;|X-XFCE-Commands=/usr/local/bin/chromium-browser;|" /usr/share/xfce4/helpers/chromium.desktop 2>/dev/null || true
        sed -i "s|X-XFCE-CommandsWithParameter=%B|X-XFCE-CommandsWithParameter=/usr/local/bin/chromium-browser|" /usr/share/xfce4/helpers/chromium.desktop 2>/dev/null || true
        mkdir -p /root/.config/xfce4
        echo "WebBrowser=chromium" > /root/.config/xfce4/helpers.rc
    '
    
    # Configure XFCE panels: top bar + bottom dock
    log_info "Configuring XFCE panels..."
    $LXC_CMD exec "$container" -- sh -c '
        mkdir -p /root/.config/xfce4/xfconf/xfce-perchannel-xml
        cat > /root/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml << '\''PANELEOF'\''
<?xml version="1.0" encoding="UTF-8"?>
...
PANELEOF
    ...
    # Install better theme and icons
    log_info "Installing desktop theme..."
    $LXC_CMD exec "$container" -- apk add --no-cache greybird-themes adwaita-icon-theme imagemagick 2>/dev/null || true
    
    # Replace default XFCE wallpaper with a solid dark color
    log_info "Setting dark desktop background..."
    $LXC_CMD exec "$container" -- sh -c '
        # Create a solid dark gray background image (1920x1080)
        convert -size 1920x1080 xc:"#1e2128" /usr/share/backgrounds/xfce/xfce-mouserace.svg 2>/dev/null || \
        convert -size 1920x1080 xc:"rgb(30,33,40)" /usr/share/backgrounds/xfce/xfce-mouserace.svg 2>/dev/null || \
        echo "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==" | base64 -d > /usr/share/backgrounds/xfce/xfce-mouserace.svg
        
        # Also replace xfce-teal which is sometimes the default
        cp /usr/share/backgrounds/xfce/xfce-mouserace.svg /usr/share/backgrounds/xfce/xfce-teal.svg 2>/dev/null || true
    '
    
    # Configure XFCE desktop appearance
    log_info "Configuring desktop appearance..."
    $LXC_CMD exec "$container" -- sh -c '
        mkdir -p /root/.config/xfce4/xfconf/xfce-perchannel-xml
        cat > /root/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-desktop.xml << '\''DESKTOPEOF'\''
<?xml version="1.0" encoding="UTF-8"?>
...
GTKEOF
        mkdir -p /root/.config/xfce4/xfconf/xfce-perchannel-xml
        cat > /root/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml << '\''XSETEOF'\''
<?xml version="1.0" encoding="UTF-8"?>
...
XSETEOF
    ...
    # Configure xfwm4 (window manager) theme
    $LXC_CMD exec "$container" -- sh -c '
        mkdir -p /root/.config/xfce4/xfconf/xfce-perchannel-xml
        cat > /root/.config/xfce4/xfconf/xfce-perchannel-xml/xfwm4.xml << '\''XFWMEOF'\''
<?xml version="1.0" encoding="UTF-8"?>
...
XFWMEOF
    ...
    log_info "Creating VNC auto-start service..."
    $LXC_CMD exec "$container" -- sh -c 'cat > /etc/init.d/xvfb << '\''XVFBEOF'\''
#!/sbin/openrc-run

name="Xvfb"
description="X Virtual Framebuffer"
command="/usr/bin/Xvfb"
command_args=":0 -screen 0 1280x720x24"
command_background="yes"
...
XVFBEOF'
    $LXC_CMD exec "$container" -- chmod +x /etc/init.d/xvfb
    
    $LXC_CMD exec "$container" -- sh -c 'cat > /etc/init.d/x11vnc << '\''VNCEOF'\''
#!/sbin/openrc-run

name="x11vnc"
description="X11 VNC Server"
command="/usr/bin/x11vnc"
command_args="-display :0 -nopw -forever -shared -rfbport 5900 -xkb"
command_background="yes"
...
VNCEOF'
    $LXC_CMD exec "$container" -- chmod +x /etc/init.d/x11vnc
    
    # Create XFCE init script for autostart on boot
    $LXC_CMD exec "$container" -- sh -c 'cat > /etc/init.d/xfce4 << '\''XFCEEOF'\''
#!/sbin/openrc-run
...
XFCEEOF'
    ...
    log_info "Enabling VNC and XFCE services..."
    $LXC_CMD exec "$container" -- rc-update add xvfb default
    $LXC_CMD exec "$container" -- rc-update add x11vnc default
    $LXC_CMD exec "$container" -- rc-update add xfce4 default
    ...
    $LXC_CMD exec "$container" -- sh -c 'cat > /root/index.php << '\''PHPEOF'\''
<?php
/**
                ...
                <div class="space-y-4 text-purple-100 leading-relaxed">
                    ...
                    <div class="mt-6 pt-4 border-t border-purple-400/30">
                        <div class="grid md:grid-cols-4 gap-4 text-center text-sm">
                            <a href="https://silverqueen.pro/masterclass" class="bg-white/5 rounded-lg p-3 hover:bg-white/10 transition-all hover:scale-105 block">
                                <div class="text-2xl mb-1">🎓</div>
                                <div class="font-medium text-purple-200">Masterclasses</div>
                                <div class="text-gray-400">Expert-led deep dives</div>
                            </a>
                            <a href="https://silverqueen.pro/courses" class="bg-white/5 rounded-lg p-3 hover:bg-white/10 transition-all hover:scale-105 block">
                                <div class="text-2xl mb-1">📚</div>
                                <div class="font-medium text-purple-200">Courses</div>
                                <div class="text-gray-400">Structured learning paths</div>
                            </a>
                            <a href="https://silverqueen.pro/certifications" class="bg-white/5 rounded-lg p-3 hover:bg-white/10 transition-all hover:scale-105 block">
                                <div class="text-2xl mb-1">🏆</div>
                                <div class="font-medium text-purple-200">Certifications</div>
                                <div class="text-gray-400">Industry-recognized credentials</div>
                            </a>
                            <a href="https://github.com/oliverbob/silverqueen.pro" target="_blank" class="bg-white/5 rounded-lg p-3 hover:bg-white/10 transition-all hover:scale-105 block">
...
PHPEOF'
    ...
    $LXC_CMD exec "$container" -- sh -c 'cat > /root/README.md << EOF
# 🚀 Ginto Sandbox
...
- \`/root/\` - Your web root (files served via Caddy)
- \`/root/index.php\` - Default landing page
- \`/root/Websites/\` - Additional web projects
EOF'
...
EOF'
    ...
    $LXC_CMD exec "$container" -- chown -R caddy:caddy /var/log/caddy
    ...
    # Stop template container - users will copy from this container directly
    # No need to publish as image - lxc copy is simpler and more efficient
    log_info "Stopping template container..."
    $LXC_CMD stop "$container"
    
    log_success "Template container '$container' ready (stopped)"
    log_success "User sandboxes will be created via: lxc copy $container <user-sandbox>"
    
    # Show current state
    $LXC_CMD list "$container" 2>/dev/null || true
    
    # Write completion status for web notification
    update_install_status "complete" "ready" "Congratulations! The sandbox template is ready. You can now create sandboxes for users." "true"
    log_info "Installation complete! Sandbox system is ready."
}
...
cmd_create() {
    ...
    if container_exists "$name"; then
        ...
        log_info "Creating sandbox '$name' from $BASE_CONTAINER (tier: $tier)..."
        log_info "Assigning static IP: $static_ip"
        
        # Copy from template container and start
        $LXC_CMD copy "$BASE_CONTAINER" "$name"
        $LXC_CMD start "$name"
    ...
    # Ensure XFCE autostart is configured and services are running
    ensure_xfce_autostart "$name"
...
bootstrap_alpine() {
    ...
    # Auto-detect latest PHP version available in Alpine
    local php_ver=$(apk search php8 | grep -oE 'php8[0-9]' | sort -ru | head -1)
    if [[ -z "$php_ver" ]]; then
        php_ver="php84"  # Fallback to PHP 8.4
        log_warn "Could not detect PHP version, using $php_ver"
    else
    ...
    apk add --no-cache \
        ...
        ${php_ver}-pdo ${php_ver}-pdo_mysql ${php_ver}-pdo_sqlite ${php_ver}-curl \
        ${php_ver}-dom ${php_ver}-xml ${php_ver}-ctype ${php_ver}-session ${php_ver}-tokenizer \
        ${php_ver}-fileinfo ${php_ver}-iconv ${php_ver}-zip ${php_ver}-gd
    ...
    log_info "Setting up Caddy log directory..."
    mkdir -p /var/log/caddy
    chown -R caddy:caddy /var/log/caddy
    
    log_info "Installing editors and tools..."
    apk add --no-cache vim nano bash curl ca-certificates openrc iputils net-tools
    
    log_info "Installing document creation tools (pandoc, wkhtmltopdf, xvfb)..."
    apk add --no-cache pandoc wkhtmltopdf xvfb-run font-noto ttf-dejavu fontconfig || true
    fc-cache -f || true
    ...
    # Create Chromium wrapper for root user (needs --no-sandbox)
    log_info "Configuring Chromium for root..."
    cat > /usr/local/bin/chromium-browser << 'CHROMEOF'
#!/bin/sh
exec /usr/bin/chromium --no-sandbox --disable-gpu --disable-software-rasterizer --disable-dev-shm-usage "$@"
CHROMEOF
    chmod +x /usr/local/bin/chromium-browser
    sed -i "s|Exec=chromium|Exec=/usr/local/bin/chromium-browser|g" /usr/share/applications/chromium.desktop 2>/dev/null || true
    ...
    # Configure XFCE panel without notification area to avoid "lost selection" warning
...
PANELEOF
    ...
    log_info "Creating VNC auto-start services..."
    cat > /etc/init.d/xvfb << 'XVFBEOF'
#!/sbin/openrc-run
...
setup_home_directory() {
    log_info "Creating /root directory structure..."
    mkdir -p /root/{Desktop,Documents,Downloads,Music,Pictures,Videos,Websites}
    
    log_info "Creating index.php..."
    cat > /root/index.php << 'PHPEOF'
<?php
/**
 * Welcome to your Ginto Sandbox!
 *
...
EOF
    ...
    log_info "Setting permissions..."
    chown -R nobody:nogroup /root 2>/dev/null || chown -R nobody:nobody /root
...
#-------------------------------------------------------------------------------
# Command: upgrade
# Upgrade existing sandbox containers with new packages (e.g., VNC support)
# Usage: ginto.sh upgrade [container|all]
#-------------------------------------------------------------------------------
cmd_upgrade() {
    local target="${1:-all}"
    local count=0
    ...
    if [[ "$target" == "all" ]]; then
        log_info "Upgrading all sandbox containers with latest packages..."
        containers=$($LXC_CMD list "${SANDBOX_PREFIX}" --format csv -c n,s | grep ",RUNNING" | cut -d',' -f1)
    else
        containers="$target"
    fi
    ...
    for container in $containers; do
        ...
        local status
        status=$($LXC_CMD info "$container" 2>/dev/null | grep "Status:" | awk '{print $2}')
        
        if [[ "$status" != "RUNNING" ]]; then
            log_warn "Container not running, starting: $container"
            $LXC_CMD start "$container" 2>/dev/null || true
            sleep 2
        fi
        ...
        log_info "  Installing VNC server and X11..."
        $LXC_CMD exec "$container" -- apk update 2>/dev/null || true
        $LXC_CMD exec "$container" -- apk add --no-cache \
            xvfb x11vnc xfce4 xfce4-terminal dbus dbus-x11 mesa-dri-gallium \
            font-noto-cjk ttf-freefont adwaita-icon-theme chromium 2>&1 | tail -5 || true
...
CHROMEOF'
        ...
        # Configure XFCE panel without notification area to avoid "lost selection" warning
        log_info "  Configuring XFCE panel..."
        $LXC_CMD exec "$container" -- sh -c 'mkdir -p /root/.config/xfce4/xfconf/xfce-perchannel-xml && cat > /root/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml << '\''PANELEOF'\''
<?xml version="1.0" encoding="UTF-8"?>
<channel name="xfce4-panel" version="1.0">
  <property name="configver" type="int" value="2"/>
  <property name="panels" type="array">
...
PANELEOF'
        ...
        # Create VNC auto-start services
        log_info "  Creating VNC services..."
        $LXC_CMD exec "$container" -- sh -c 'cat > /etc/init.d/xvfb << '\''XVFBEOF'\''
#!/sbin/openrc-run
...
XFCEEOF'
        ...
        log_info "  Enabling and starting VNC services..."
        $LXC_CMD exec "$container" -- rc-update add xvfb default 2>/dev/null || true
        $LXC_CMD exec "$container" -- rc-update add x11vnc default 2>/dev/null || true
        $LXC_CMD exec "$container" -- rc-update add xfce4 default 2>/dev/null || true
        $LXC_CMD exec "$container" -- rc-service xvfb start 2>/dev/null || true
        sleep 2
        $LXC_CMD exec "$container" -- rc-service x11vnc start 2>/dev/null || true
        ...
        count=$((count + 1))
        log_success "  Upgraded: $container"
    done
    
    if [[ $count -eq 0 ]]; then
        log_warn "No containers to upgrade"
    else
        log_success "Upgraded $count container(s)"
    fi
}
...
cmd_verify() {
    ...
    # Check template container (used for lxc copy)
    echo -n "Template container ($BASE_CONTAINER): "
    if container_exists "$BASE_CONTAINER"; then
        local state
        state=$($LXC_CMD info "$BASE_CONTAINER" 2>/dev/null | grep -i "status:" | awk '{print $2}')
        echo -e "${GREEN}OK${NC} ($state)"
    else
        echo -e "${RED}MISSING${NC} - run: bin/ginto.sh install"
    fi
...
#-------------------------------------------------------------------------------
# Command: network
# Configure network mode for sandbox containers
# Usage: ginto.sh network [bridge|nat|macvlan|status]
#-------------------------------------------------------------------------------
cmd_network() {
    local mode="${1:-status}"
    local script="${SCRIPT_DIR}/setup_network.sh"
    
    if [[ ! -f "$script" ]]; then
        die "setup_network.sh not found at $script"
    fi
    
    case "$mode" in
        bridge)
            log_info "Switching to bridge network mode..."
            echo ""
            echo "Bridge mode uses the default LXD bridge (lxdbr0)"
            echo "IP Range: 10.0.0.0/8 (16.7 million unique IPs)"
            echo "NAT: Disabled (direct routing)"
            echo ""
            ...
            sudo bash "$script" bridge
            echo ""
            log_success "Network mode set to: bridge"
            log_info "Restart sandbox-proxy for changes to take effect"
            ;;
        nat)
            log_info "Switching to NAT network mode..."
            echo ""
            echo "NAT mode uses LXD bridge with Network Address Translation"
            echo "IP Range: 10.0.0.0/8 (16.7 million unique IPs)"
            echo "NAT: Enabled (outbound traffic uses host's public IP)"
            echo ""
            echo "Benefits of NAT:"
            echo "  - Familiar Docker-like networking"
            echo "  - Containers hidden behind host IP"
            echo "  - Simple firewall rules"
            echo ""
            read -p "Continue? [y/N] " confirm
            if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then
                log_warn "Cancelled"
            ...
            sudo bash "$script" nat
            echo ""
            log_success "Network mode set to: nat"
            log_info "Restart sandbox-proxy for changes to take effect"
            ;;
        macvlan)
            log_info "Switching to macvlan network mode..."
            echo ""
            echo "Macvlan mode enables full 32-bit IP address space"
            echo "IP Range: 4.3 billion unique IPs (excluding reserved)"
            echo ""
            echo "This will:"
            echo "  1. Create ginto-dummy0 interface with /0 CIDR"
            echo "  2. Create ginto-shim interface for host routing"
            echo "  3. Create ginto-macvlan LXD network"
            echo "  4. Set up systemd persistence"
            echo ""
            read -p "Continue? [y/N] " confirm
            if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then
                log_warn "Cancelled"
            ...
            sudo bash "$script" macvlan
            echo ""
            log_success "Network mode set to: macvlan"
            log_info "Restart sandbox-proxy for changes to take effect"
            ;;
        ipvlan)
            log_info "Switching to ipvlan network mode..."
            echo ""
            echo "IPVLAN mode enables full 32-bit IP address space"
            echo "IP Range: 4.3 billion unique IPs (excluding reserved)"
            echo "Layer: L3 (IP routing, no broadcast)"
            echo ""
            echo "Benefits of IPVLAN:"
            echo "  - Works in cloud environments with MAC restrictions"
            echo "  - Lower overhead than macvlan"
            echo "  - Better for nested virtualization"
            echo "  - No broadcast/multicast traffic"
            echo ""
            read -p "Continue? [y/N] " confirm
            if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then
                log_warn "Cancelled"
            ...
            sudo bash "$script" ipvlan
            echo ""
            log_success "Network mode set to: ipvlan"
            log_info "Restart sandbox-proxy for changes to take effect"
            ;;
        status)
            bash "$script" status
            ;;
        teardown)
            log_warn "This will remove all macvlan/ipvlan infrastructure"
            read -p "Continue? [y/N] " confirm
            if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then
                log_warn "Cancelled"
                return 1
            fi
            sudo bash "$script" teardown
            log_success "Network infrastructure removed, mode set to bridge"
            ;;
        *)
            echo "Usage: ginto.sh network [bridge|nat|macvlan|ipvlan|status|teardown]"
            echo ""
            echo "Commands:"
            echo "  bridge    Switch to bridge mode (16.7M IPs, direct routing)"
            echo "  nat       Switch to NAT mode (16.7M IPs, NAT via host)"
...
# Log file for background installations
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --background|--bg|--daemon)
                ...
                ;;
            --auto)
                # Auto mode for web terminal - run in background with logging
                # Re-exec ourselves with nohup if not already backgrounded
                if [[ "${GINTO_BACKGROUNDED:-}" != "1" ]]; then
                    export GINTO_BACKGROUNDED=1
                    shift
                    # Get remaining args (including command like 'install')
                    local remaining_args=("$@")
                    # Default to 'install' if no command specified
                    [[ ${#remaining_args[@]} -eq 0 ]] && remaining_args=("install")
                    
                    log_info "Starting installation in background mode..."
                    log_info "Log file: $INSTALL_LOG"
                    nohup bash "$0" "${remaining_args[@]}" > "$INSTALL_LOG" 2>&1 &
                    local pid=$!
                    echo "$pid" > "${PROJECT_ROOT}/.install.pid"
                    log_success "Installation started in background (PID: $pid)"
                    log_info "Monitor with: tail -f $INSTALL_LOG"
                    exit 0
                fi
                shift
                ;;
