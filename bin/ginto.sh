...
EOF
    ...
    # Generate sudoers content for all web users
    {
        ...
        echo "# SECURITY NOTE:"
        echo "# - Web users can run lxc commands without password"
        ...
        for user in $lxc_users; do
            ...
            echo ""
            echo "# Allow all lxc commands (container isolation provides security)"
...
create_base_image() {
    ...
    if $LXC_CMD info "$container" &>/dev/null; then
        ...
        echo ""
        echo "  Options:"
        echo "    1) Delete existing container and create fresh"
        echo "    2) Create new container with different name (e.g., ${container}-1)"
        ...
        case "$choice" in
            ...
            2)
                local i=1
                while $LXC_CMD info "${container}-${i}" &>/dev/null; do
                    ((i++))
                done
                container="${container}-${i}"
                log_info "Using alternate name: $container"
                ;;
            *)
    ...
    # Add alias to the base Alpine image for easy identification
    local alpine_fingerprint
    alpine_fingerprint=$($LXC_CMD image list "images:alpine/$ALPINE_VERSION" --format csv -c f 2>/dev/null | head -1)
    if [ -z "$alpine_fingerprint" ]; then
        # Try to get from local images (already downloaded)
        alpine_fingerprint=$($LXC_CMD image list --format csv -c f 2>/dev/null | head -1)
    fi
    if [ -n "$alpine_fingerprint" ]; then
        local short_fp="${alpine_fingerprint:0:12}"
        $LXC_CMD image alias create "gs_${short_fp}" "$alpine_fingerprint" 2>/dev/null || true
        log_info "Added alias gs_${short_fp} to Alpine base image"
    fi
...
EOF'
    ...
    update_install_status "complete" "ready" "Congratulations! The base image is ready. You can now create sandboxes for users." "true"
