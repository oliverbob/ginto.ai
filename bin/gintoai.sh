...
install_utilities() {
    ...
    case $OS in
        ubuntu|debian)
            sudo apt-get install -y curl unzip ffmpeg lsof
            ;;
        fedora)
            sudo dnf install -y curl unzip ffmpeg lsof
            ;;
    esac
...
Restart=always
RestartSec=5
StandardOutput=append:$PROJECT_DIR/logs/ginto.log
StandardError=append:$PROJECT_DIR/logs/ginto-error.log
...
setup_permissions() {
    ...
    # Ensure .env file is owned by install user and group-writable for web installer
    if [ -f "$PROJECT_DIR/.env" ]; then
        chown "$INSTALL_USER:$INSTALL_USER" "$PROJECT_DIR/.env"
        chmod 664 "$PROJECT_DIR/.env"
        log_info "Set .env ownership and permissions"
    fi
...
print_summary() {
    ...
    echo "  - Logs:    tail -f $PROJECT_DIR/logs/ginto.log"
    echo ""
}
