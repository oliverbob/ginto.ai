...
| `LXD_NETWORK_MODE` | Network mode: `bridge` (16.7M IPs) or `macvlan` (4.3B IPs) | `bridge` |
| `LXD_NETWORK_PREFIX` | If set, uses subnet mode (e.g., "10.166.3") | `null` |
...
#### Bridge Mode (Default)
- Uses LXD's `lxdbr0` bridge network
- IPs constrained to 10.0.0.0/8 private range
- **16.7 million unique IPs** (24-bit address space)
- Works in any environment (nested LXC, cloud, etc.)
- No additional setup required

#### Macvlan Mode (Full 32-bit)
- Uses macvlan network on dummy interface
- **4.3 billion unique IPs** (full 32-bit address space)
- Requires `setup_macvlan.sh` to configure infrastructure
- Uses shim interface for host-to-container communication
- Reserved ranges remapped: 0.x→1.x, 127.x→128.x, 224-255.x→64-95.x

To enable macvlan mode:
```bash
# One-time setup via ginto.sh (recommended)
bin/ginto.sh network macvlan

# Or directly via setup script
sudo ./bin/setup_network.sh macvlan

# Enable in .env (done automatically by above commands)
echo "LXD_NETWORK_MODE=macvlan" >> .env
```
