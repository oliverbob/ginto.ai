...
The chat system can perform pre-LLM web searches to fetch up-to-date information when it detects a prompt requires current or external data. For safety and to respect developer intent, automatic web searches are disabled by default unless explicitly enabled via the environment variable `AUTO_WEB_SEARCH=1`.

When `AUTO_WEB_SEARCH` is not set to `1`, the system will only perform a web search if the user's prompt explicitly requests it (e.g. "search the web for X", "look up", "find information about"). This prevents the editor from performing background web searches unless the user asks for them.
