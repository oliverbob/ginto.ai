...
final class SandboxMcp
{
    ...
    public function createDocument(
        ...
        // For PDF, use a two-step process: Markdown -> HTML -> PDF (via weasyprint)
        ...
        if ($format === 'pdf') {
            ...
            if ($exitCode !== 0) {
                return [
                    'success' => false,
                    'error' => 'WeasyPrint PDF conversion failed: ' . ($stderr ?: $stdout ?: 'Unknown error'),
            ...
            if ($exitCode !== 0) {
                return [
                    'success' => false,
                    'error' => 'Pandoc conversion failed: ' . ($stderr ?: $stdout ?: 'Unknown error'),
                    'source_file' => $sourcePath,
                    'command' => $pandocCmd,
                    'exit_code' => $exitCode
                ];
