...
class UploadController extends Controller
{
    ...
    // Database instance (untyped to accept Medoo\Medoo or wrapper instance)
    protected $db = null;
                ...
                elseif (!$isDbAvailable) { $_SESSION['upload_error_flash'] = 'Database service not available for upload processing.'; }
                    ...
                    try { 
                        ...
                        $dbData = [
                            ...
                            'visibility' => 'private', 'uploaded_at_provider' => \Medoo\Medoo::raw('NOW()'),
            ...
            if (isset($_POST['update_metadata'])) {
                ...
                elseif (!$isDbAvailable) { $_SESSION['upload_error_flash'] = 'Database service unavailable for metadata update.'; }
                    ...
                    try {
                        ...
                        $updateData = ["title" => $title, "description" => $description, "tags" => $tagsJson, "visibility" => $visibility, "updated_at" => \Medoo\Medoo::raw('NOW()')];
    ...
    public function createPostWithMedia()
    {
        ...
        try {
            ...
            $cloudFileData = [
                ...
                'uploaded_at_provider' => \Medoo\Medoo::raw('NOW()'),
            ...
            $postDataForDb = [
                ...
                // Some schemas use `author_id` as the FK to users; set both for compatibility
                'author_id' => $currentUserId,
                'content' => $postContent, 'image' => $mediaUrlForPost,
                'cloud_file_id' => $cloudFileRecordId, 'visibility' => $postVisibility,
                'post_type' => 'media', 'created_at' => \Medoo\Medoo::raw('NOW()'), 'updated_at' => \Medoo\Medoo::raw('NOW()')
            ];
    ...
    private function handleUploadCoverPhoto()
    {
        ...
        try {
            ...
            $updateUserResult = $this->db->update('users', [
                ...
                'updated_at' => \Medoo\Medoo::raw('NOW()')
            ], ['id' => $currentUserId]);
