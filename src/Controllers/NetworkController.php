...
class NetworkController extends Controller
{
    ...
    public function start($name)
    {
        ...
        $output = [];
        $code = 0;
        exec("sudo $lxcBin start " . escapeshellarg($name) . " 2>&1", $output, $code);
    ...
    public function stop($name)
    {
        ...
        $output = [];
        $code = 0;
        exec("sudo $lxcBin stop " . escapeshellarg($name) . " 2>&1", $output, $code);
    ...
    public function restart($name)
    {
        ...
        $output = [];
        $code = 0;
        exec("sudo $lxcBin restart " . escapeshellarg($name) . " 2>&1", $output, $code);
    ...
    private function getAllContainers(): array
    {
        ...
        $output = [];
        $code = 0;
        // Get name, status, ipv4, type, snapshots, created_at - use sudo for web server
        exec("sudo $lxcBin list --format csv -c ns4tSc 2>/dev/null", $output, $code);
        ...
        foreach ($output as $line) {
            // Ensure $line is a string and supply explicit CSV escape char to avoid deprecation warnings
    ...
    private function getContainerInfo(string $name): ?array
    {
        ...
        $output = [];
        $code = 0;
        exec("sudo $lxcBin info " . escapeshellarg($name) . " 2>/dev/null", $output, $code);
