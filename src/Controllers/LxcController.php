...
class LxcController extends Controller
{
    ...
    public function start($name)
    {
        ...
        header('Location: /admin/lxcs/' . urlencode($name));
    }
    ...
    public function create()
    {
        ...
        if (empty($userId)) {
            ...
            header('Location: /admin/lxcs?error=User+ID+required');
            return;
        }
        ...
        if ($result['success']) {
            header('Location: /admin/lxcs/' . urlencode($result['sandboxId']));
        } else {
            header('Location: /admin/lxcs?error=' . urlencode($result['error'] ?? 'Failed to create'));
        }
    }
    ...
    private function getSystemStats(): array
    {
        ...
        // Get images count
        $images = [];
        exec("lxc image list --format csv -c f 2>/dev/null", $images, $code);
