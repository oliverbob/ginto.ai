...
class AdminLxcController
{
    ...
    public function containerAction($name, $action): void
    {
        ...
        try {
            ...
            if ($action === 'delete') {
                ...
                // Also delete from client_sandboxes database table
                // Container names are formatted as ginto-sandbox-{sandbox_id}
                if ($this->db && str_starts_with($name, 'ginto-sandbox-')) {
                    $sandboxId = substr($name, strlen('ginto-sandbox-'));
                    try {
                        $this->db->delete('client_sandboxes', ['sandbox_id' => $sandboxId]);
                        $this->db->delete('client_sandboxes', ['container_name' => $name]);
                    } catch (\Throwable $_) {
                        // Ignore DB errors - container deletion is the primary action
                    }
                }
            } else {
    ...
    public function imageDelete($fingerprint = ''): void
    {
        ...
        try {
            ...
            // Clear install status marker since the base image may have been deleted
