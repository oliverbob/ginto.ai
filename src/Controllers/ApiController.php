...
class ApiController
{
        ...
        if ($model === 'kimi_k2') {
            ...
            if (!empty($sandboxId) && class_exists('\Ginto\Helpers\LxdSandboxManager') && \Ginto\Helpers\LxdSandboxManager::sandboxExists($sandboxId)) {
                $sys .= "\n\n## SANDBOX ACCESS - AGENTIC MODE\n"
                    ...
                    . "- `sandbox_delete` - Delete file/folder (args: path)\n"
                    . "- `sandbox_create_document` - **ALWAYS USE FOR DOCUMENTS** - Create real PDF/DOCX/ODT files (args: filename, content, format=pdf|docx|odt|html|rtf|md|txt, title, folder)\n"
                    . "- `sandbox_list_document_formats` - List available document formats\n\n"
                    . "⚠️ **DOCUMENT CREATION RULE**: NEVER use sandbox_write_file + sandbox_exec for documents. ALWAYS use sandbox_create_document - it handles Pandoc/WeasyPrint automatically.\n\n"
                    . "### Tool Call Format:\n"
    ...
    public function websearch(): void
    {
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
            ...
            if ($finalContent || $accumulatedReasoning) {
                ...
                $html = $parsedown ? $parsedown->text(\Ginto\Handlers\ChatStreamHandler::fixCodeBlockNewlines($finalContent)) : nl2br(htmlspecialchars($finalContent));
    ...
    public function models(): void
    {
        ...
        if (empty($runningModels)) {
            try {
                ...
                $ch = curl_init("http://{$host}:11434/api/ps");
    ...
    public function modelsSet(): void
    {
        header('Content-Type: application/json; charset=utf-8');
        if (session_status() !== PHP_SESSION_ACTIVE) @session_start();
