...
class StoriesController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if (isset($GLOBALS['db']) && $GLOBALS['db']) {
            ...
            try {
                ...
                $this->db = null;
                error_log('StoriesController: could not initialize DB instance: ' . $e->getMessage());
            }
        } else {
            $this->db = null;
        }
    ...
    public function createStoryWithMedia()
    {
        ...
        // Require auth for mutating action
        $this->requireAuth();
        ...
        try {
            ...
            // Application log for story creation
            $this->_app_log("Story created: id={$newStoryId} user={$currentUserId} file={$originalFileNameForLog}", 'info');
            ...
            $this->_app_log($detailedError, 'error');
    ...
    /**
     * Legacy route wrapper: POST /post/stories/create
     */
    public function postStoriesCreate()
    {
        $this->requireAuth();
        $this->createStoryWithMedia();
    }
    ...
    public function deleteStory(int $storyId)
    {
        ...
        // Require authentication for mutating action
        $this->requireAuth();
        $currentUserId = (int) ($_SESSION['user_id'] ?? 0);
        ...
        try {
            ...
            // Application log for story deletion
            $this->_app_log("Story deleted: id={$storyId} user={$currentUserId}", 'info');
            ...
            $this->_app_log("DeleteStory Exception (StoryID: {$storyId}, User: {$currentUserId}): " . $e->getMessage(), 'error');
            if ($response['message'] === 'An unexpected error occurred.') {
                if (http_response_code() < 400) http_response_code(500);
    ...
    /**
     * Application-level logger that writes to storage/logs/ginto.log
     */
    private function _app_log(string $message, string $level = 'info'): void
    {
        try {
            $path = __DIR__ . '/../../storage/logs/ginto.log';
            $ts = date('Y-m-d H:i:s');
            $entry = sprintf("[%s] %s: %s\n", $ts, strtoupper($level), $message);
            $res = @file_put_contents($path, $entry, FILE_APPEND | LOCK_EX);
            if ($res === false) {
                error_log("APP LOG FAIL: " . $entry);
            }
        } catch (\Throwable $t) {
            // Never let logging throw
            error_log("APP LOG EXCEPTION: " . $t->getMessage());
        }
    }

    /**
     * Require authenticated session for mutating endpoints.
     * Exits with 401 JSON payload when not authenticated.
     */
