    ...
    public function onOpen(ConnectionInterface $conn)
    {
        ...
        if ($sessionId && isset(self::$sessions[$sessionId])) {
            ...
            if (!$status['running']) {
                ...
                error_log("[PtyServer] Session {$sessionId} process died, destroying session");
                $this->destroySession($sessionId);
            } else {
                // Rejoin existing session
                error_log("[PtyServer] Reconnecting to session: {$sessionId}");
                
                // Disconnect any existing connection for this session (if different)
                if ($session['conn'] && $session['conn'] !== $conn) {
                    $oldConn = $session['conn'];
                    $session['conn'] = null; // Clear first to prevent reconnect loop
                    
                    // Send a "replaced" message so client knows not to reconnect
                    try { 
                        $oldConn->send("\r\n\033[33m*** Session taken over by another connection ***\033[0m\r\n");
                        $oldConn->send(json_encode(['type' => 'replaced']));
                    } catch (\Throwable $_) {}
                    
                    // Close the old connection
                    try { $oldConn->close(); } catch (\Throwable $_) {}
                }
        ...
        if ($cmd === null) {
            // OS mode: Run shell as the web app user, not root
            // Always use oliverbob for security - never run as root
            $webUser = 'oliverbob';
            
            // If running as root, switch to the web app user
            if (posix_getuid() === 0) {
                // Use su to switch to the correct user
                $cmd = ['script', '-q', '-c', "su - {$webUser}", '/dev/null'];
                error_log("[PtyServer] OS mode as root, switching to user: {$webUser}");
            } else {
        ...
        try {
            if (is_string($msg) && strlen($msg) && $msg[0] === '{') {
                ...
                if (is_array($j) && !empty($j['type'])) {
                    ...
                    if ($j['type'] === 'ping') {
                        // Send pong response to keep connection alive through proxies/firewalls
                        try { $from->send(json_encode(['type' => 'pong'])); } catch (\Throwable $_) {}
                        return;
                    }
                }
            }
