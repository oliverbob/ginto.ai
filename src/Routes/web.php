...
$router->req('/sandbox/image-install-status', function() use ($db) {
    ...
    $statusFile = dirname(__DIR__, 2) . '/../storage/.image_install_status';
    // Normalize the path - ginto.ai is at dirname(__DIR__, 2), storage is sibling to ginto.ai
    $gintoRoot = dirname(__DIR__, 2);
    $statusFile = dirname($gintoRoot) . '/storage/.image_install_status';
    $logFile = $gintoRoot . '/install.log';
    
    if (!file_exists($statusFile)) {
        // No installation in progress or never started
        echo json_encode([
            'success' => true,
            'status' => 'not_started',
    ...
    if (file_exists($logFile)) {
        ...
        $logTail = implode('', array_slice($logLines, -20)); // Last 20 lines
    }
...
$router->req('/admin/lxc', function() use ($db) {
    ...
    include __DIR__ . '/../Views/admin/lxc/lxc.php';
    exit;
});
...
$router->req('/admin/api/lxc/containers', function() use ($db) {
    ...
    // GET = List containers
    try {
        // Check if LXD is available - check common paths since web server PATH may differ
        $lxcBin = getLxcBin();
        if (!$lxcBin) {
        ...
        foreach ($containers as $c) {
            ...
            // Get disk usage - LXD state often shows 0 for ZFS/btrfs storage
            // So we try to get actual disk usage via df for running containers
            ...
            if ($diskBytes > 0) {
                ...
                // Try to get actual disk usage from running container via df
                // df -B1 output: Filesystem Size Used Available Use% Mounted
                // But filesystem name may be on separate line, so just match the numbers
                $dfOutput = shell_exec("sudo $lxcBin exec " . escapeshellarg($c['name']) . " -- df -B1 / 2>/dev/null | tail -1");
                // Match: Total Used Available (3 consecutive numbers)
                if (preg_match('/(\d+)\s+(\d+)\s+(\d+)\s+\d+%/', $dfOutput ?? '', $m)) {
                    $diskTotal = intval($m[1]); // Total/Size
                    $diskBytes = intval($m[2]); // Used
                    // $m[3] is Available (not needed)
                }
                if ($diskBytes > 0) {
                    $disk = round($diskBytes / 1024 / 1024) . ' MB';
                }
            ...
            // Get container limits from config
            $limits = [
            ...
            // Get disk limit from root device config, or from df total
...
$router->get('/admin/api/lxc/stats', function() use ($db) {
    if (session_status() !== PHP_SESSION_ACTIVE) {@session_start();}
    header('Content-Type: application/json; charset=utf-8');
    ...
    try {
        ...
        if ($memInfo) {
            if (preg_match('/MemTotal:\s+(\d+)/', $memInfo, $m)) {
                $memTotal = (int)$m[1] * 1024; // Convert KB to bytes
            }
...
$router->req('/admin/api/lxc/prune', function() use ($db) {
    ...
    try {
        ...
        // Get all containers to find used images
        $containersOutput = shell_exec("sudo $lxcBin list --format json 2>/dev/null");
        $containers = json_decode($containersOutput, true) ?: [];
        
        // For now, just report what would be pruned (LXD doesn't have a built-in prune)
