...
// Normalize global variables to ensure valid types
$currentPath = is_string($currentPath) ? $currentPath : '/';
$ICON_ACTIVE_CLASS = is_string($ICON_ACTIVE_CLASS) ? $ICON_ACTIVE_CLASS : 'text-yellow-500 dark:text-yellow-300';
$ICON_SVG_BASE_CLASS = is_string($ICON_SVG_BASE_CLASS) ? $ICON_SVG_BASE_CLASS : 'stroke-current fill-current';
$ICON_ACTIVE_HEX = is_string($ICON_ACTIVE_HEX) ? $ICON_ACTIVE_HEX : '#f59e0b';
$ICON_DEFAULTS = is_array($ICON_DEFAULTS) ? $ICON_DEFAULTS : [];
$ICON_DEFAULT_HEX = is_array($ICON_DEFAULT_HEX) ? $ICON_DEFAULT_HEX : [];
 ...
 *
 * @param string|null $path Route prefix to consider active
...
function activeIconClass(?string $path, ?string $fallback = null, ?string $active = null) : string
{
    ...
    $best = '';
    if (is_array($ICON_DEFAULTS)) {
        foreach ($ICON_DEFAULTS as $k => $v) {
            if (is_string($k) && is_string($path) && str_starts_with($path, $k) && strlen($k) > strlen($best)) {
                $best = $k;
            }
        }
    }
 ...
 *
 * @param string|null $path Route prefix to consider
 * @param string|null $fallbackHex optional fallback color hex (e.g. "#999")
