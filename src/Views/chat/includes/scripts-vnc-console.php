<?php
/**
 * VNC Desktop, Console Tab, and Ginto Cloud Tab Management Scripts
 */
?>
<script>
  // ========================================
  // My Files / My Computer / Console / Cloud Tab Switching
  // ========================================
  ...
  const cloudTabContainer = document.getElementById('cloud-tab-container');
  const cloudTabStatus = document.getElementById('cloud-tab-status');
  const cloudTabCountdown = document.getElementById('cloud-tab-countdown');
  
  let vncIframeUser = null;
  let vncConnected = false;
  let currentEditorTab = 'files'; // 'files', 'computer', 'console', or 'cloud'
  let currentSandboxBackend = 'lxd'; // 'lxd' or 'docker' - affects available tabs
  
  // Cloud tab state
  let cloudSubdomain = null;
  let cloudExpiry = null;
  let cloudCountdownInterval = null;
  let cloudIframe = null;
  ...
  function updateTabsForBackend(backend) {
    ...
    if (backend === 'docker') {
      ...
      console.log('[Sandbox] Docker mode: VNC, Console, and Cloud tabs hidden');
    } else {
      ...
      if (tabCloud) tabCloud.classList.remove('hidden');
  ...
  function switchToFilesTab() {
    ...
    if (tabCloud) setTabInactive(tabCloud);
    ...
    if (cloudTabContainer) cloudTabContainer.classList.add('hidden');
    ...
    if (cloudTabStatus) cloudTabStatus.classList.add('hidden');
    if (cloudTabCountdown) cloudTabCountdown.classList.add('hidden');
  }
  ...
  function switchToComputerTab() {
    ...
    if (tabConsole) setTabInactive(tabConsole);
    if (tabCloud) setTabInactive(tabCloud);
    // Hide editor, console, and cloud; show VNC
  ...
  function switchToConsoleTab() {
    ...
    if (tabMyComputer) setTabInactive(tabMyComputer);
    if (tabCloud) setTabInactive(tabCloud);
    // Hide editor, VNC, and cloud; show console
    if (editorIframe) editorIframe.classList.add('hidden');
    if (vncDesktopContainer) vncDesktopContainer.classList.add('hidden');
  ...
  function switchToCloudTab() {
    currentEditorTab = 'cloud';
    // Update tab styling
    if (tabCloud) setTabActive(tabCloud);
    if (tabMyFiles) setTabInactive(tabMyFiles);
    if (tabMyComputer) setTabInactive(tabMyComputer);
    if (tabConsole) setTabInactive(tabConsole);
    // Hide editor, VNC, and console; show cloud
    if (editorIframe) editorIframe.classList.add('hidden');
    if (vncDesktopContainer) vncDesktopContainer.classList.add('hidden');
    if (consoleTabContainer) consoleTabContainer.classList.add('hidden');
    if (cloudTabContainer) cloudTabContainer.classList.remove('hidden');
    if (vncUserStatus) vncUserStatus.classList.add('hidden');
    if (consoleTabStatus) consoleTabStatus.classList.add('hidden');
    if (consoleTabAddBtn) consoleTabAddBtn.classList.add('hidden');
    if (cloudTabStatus) cloudTabStatus.classList.remove('hidden');
    if (cloudSubdomain && cloudExpiry > Date.now()) {
      if (cloudTabCountdown) cloudTabCountdown.classList.remove('hidden');
    }
    
    // Register cloud subdomain if not already registered or expired
    if (!cloudSubdomain || cloudExpiry <= Date.now()) {
      registerCloudSubdomain();
    }
  }
  ...
  async function connectConsoleTerminal(tabObj) {
    ...
    ws.onmessage = (event) => {
      try {
        // Filter out pong/control messages
        if (typeof event.data === 'string' && event.data.startsWith('{')) {
          try {
            const msg = JSON.parse(event.data);
            if (msg.type === 'pong' || msg.type === 'replaced') return;
          } catch (_) {}
        }
  ...
  tabCloud?.addEventListener('click', switchToCloudTab);
  consoleTabAddBtn?.addEventListener('click', createConsoleTerminalTab);
  
  // ========================================
  // Ginto Cloud Tab Functions
  // ========================================
  
  function updateCloudStatus(status, colorClass) {
    if (cloudTabStatus) {
      cloudTabStatus.textContent = status;
      cloudTabStatus.className = 'text-xs px-2 py-0.5 rounded-full ' + colorClass;
    }
  }
  ...
  function startCloudCountdown() {
    // Clear any existing countdown
    if (cloudCountdownInterval) {
      clearInterval(cloudCountdownInterval);
    }
    
    const countdownEl = document.getElementById('cloud-tab-countdown');
    const timerDisplay = document.getElementById('cloud-timer-display');
    
    function updateCountdown() {
      const remaining = cloudExpiry - Date.now();
      if (remaining <= 0) {
        clearInterval(cloudCountdownInterval);
        cloudCountdownInterval = null;
        
        // Update UI to show expired
        if (countdownEl) {
          countdownEl.textContent = '⏱ Expired';
          countdownEl.classList.remove('bg-yellow-600', 'text-yellow-100');
          countdownEl.classList.add('bg-red-600', 'text-red-100');
        }
        if (timerDisplay) {
        ...
        updateCloudStatus('Expired', 'bg-red-600 text-red-100');
    ...
    // Update every second
    cloudCountdownInterval = setInterval(updateCountdown, 1000);
  }
  
  async function registerCloudSubdomain() {
    const loadingEl = document.getElementById('cloud-loading');
    const loadingText = document.getElementById('cloud-loading-text');
    const contentEl = document.getElementById('cloud-content');
    const subdomainDisplay = document.getElementById('cloud-subdomain-display');
    const subdomainLink = document.getElementById('cloud-subdomain-link');
    const copyBtn = document.getElementById('cloud-copy-url');
    const timerDisplay = document.getElementById('cloud-timer-display');
    const refreshBtn = document.getElementById('cloud-refresh-btn');
    
    // Show loading state
    if (loadingEl) loadingEl.classList.remove('hidden');
    if (loadingText) loadingText.textContent = 'Setting up your cloud subdomain...';
    updateCloudStatus('Registering...', 'bg-yellow-600 text-yellow-100');
    
    try {
      const csrfToken = window.GINTO_AUTH?.csrfToken || '';
      const res = await fetch('/api/sandbox/cloud/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({ csrf_token: csrfToken })
      ...
      if (!res.ok || !data.success) {
        if (loadingText) loadingText.textContent = data.error || 'Failed to register cloud subdomain';
        updateCloudStatus('Error', 'bg-red-600 text-red-100');
        if (refreshBtn) refreshBtn.classList.remove('hidden');
        return;
      }
      
      // Store subdomain info
      cloudSubdomain = data.subdomain;
      cloudExpiry = data.expires_at * 1000; // Convert to milliseconds
      ...
      // Show subdomain info
      if (subdomainDisplay) subdomainDisplay.classList.remove('hidden');
      if (subdomainLink) {
        subdomainLink.href = cloudUrl;
        subdomainLink.textContent = cloudUrl.replace('https://', '');
      }
      if (timerDisplay) timerDisplay.classList.remove('hidden');
      if (refreshBtn) refreshBtn.classList.add('hidden');
      
      // Update status
      updateCloudStatus('Active', 'bg-green-600 text-green-100');
      if (cloudTabCountdown) {
        cloudTabCountdown.classList.remove('hidden');
        cloudTabCountdown.classList.remove('bg-red-600', 'text-red-100');
        cloudTabCountdown.classList.add('bg-yellow-600', 'text-yellow-100');
      }
      ...
      cloudIframe = document.createElement('iframe');
      cloudIframe.src = cloudUrl;
      cloudIframe.className = 'absolute inset-0 w-full h-full border-0';
      cloudIframe.allow = 'fullscreen';
      contentEl.appendChild(cloudIframe);
      ...
      console.log('[Cloud] Subdomain registered:', cloudSubdomain, 'expires at:', new Date(cloudExpiry));
      
    } catch (err) {
      console.error('[Cloud] Registration error:', err);
      if (loadingText) loadingText.textContent = 'Failed to connect: ' + err.message;
      updateCloudStatus('Error', 'bg-red-600 text-red-100');
      if (refreshBtn) refreshBtn.classList.remove('hidden');
    }
  }
  
  // Cloud refresh button handler
