...
(function() {
  ...
  const PING_INTERVAL_MS = 15000;  // Send ping every 15 seconds to prevent idle disconnects
  ...
  function saveConsoleState() {
    try {
      // Save session IDs so we can reconnect after page reload
      const sessionIds = tabs.map(t => ({ sessionId: t.sessionId, label: t.tabBtn?.querySelector('.tab-title')?.textContent || 'Terminal' }));
  ...
  function loadConsoleState() {
    try {
      ...
      if (data.isMinimized && data.hasOpenTabs) {
        ...
        // Also restore the sessions (they'll reconnect when clicked)
        if (data.sessions && data.sessions.length > 0) {
          data.sessions.forEach(s => {
            if (s.sessionId) {
              // Create tab structure but don't connect yet
              createTab(null, null, { sessionId: s.sessionId, tabLabel: s.label || 'Terminal' });
            }
          });
        }
      }
   ...
   * @param {string|null} initialCommand - Command to run after connection
   * @param {string|null} targetMode - 'sandbox' or null (default to host for admin)
   * @param {object} options - Additional options
   * @param {string} options.sessionId - Custom session ID for persistence (e.g., 'ginto-install')
   * @param {string} options.tabLabel - Custom tab label
   */
  function createTab(initialCommand, targetMode = null, options = {}) {
    tabCounter++;
    const tabId = 'tab-' + tabCounter;
    const tabLabel = options.tabLabel || 'Terminal ' + tabCounter;
    ...
    // Use custom session ID if provided (for persistent sessions like install)
  ...
  async function connectTab(tab, targetMode = null) {
    ...
    tab.ws.addEventListener('open', function() {
      ...
      // Send resize event to force terminal redraw (helps screen/tmux redraw after reconnect)
      setTimeout(function() {
        if (tab.ws && tab.ws.readyState === WebSocket.OPEN) {
          tab.fitAddon.fit();
      ...
      try {
        ...
        if (typeof e.data === 'string' && e.data.startsWith('{')) {
          try {
            ...
            if (msg.type === 'pong') return; // Silent keepalive response
            if (msg.type === 'replaced') {
              // This connection was replaced by another - don't reconnect
              tab.autoReconnect = false;
              tab.term.write('\r\n\x1b[33m*** Connection replaced - refresh to reconnect ***\x1b[0m\r\n');
              return;
            }
          } catch (_) {}
        }
  ...
  /**
   * Open console with a command - creates new tab or reconnects to existing session
   * @param {string} command - Command to run after connection
   * @param {string|null} targetMode - 'sandbox' or null (default to host for admin)
   * @param {object} options - Additional options
   * @param {string} options.sessionId - Custom session ID for persistence
   * @param {string} options.tabLabel - Custom tab label
   */
  window.openConsoleWithCommand = function(command, targetMode = null, options = {}) {
    ...
    // If a sessionId is provided, check if we already have a tab with this session
    if (options.sessionId) {
      const existingTab = tabs.find(t => t.sessionId === options.sessionId);
      if (existingTab) {
        // Switch to existing tab and reconnect if needed
        switchToTab(existingTab.id);
        if (!existingTab.ws || existingTab.ws.readyState !== WebSocket.OPEN) {
          // Force reconnect
          if (existingTab.ws) existingTab.ws.close();
          existingTab.ws = null;
          connectTab(existingTab, targetMode);
        }
        saveConsoleState();
        return;
      }
    ...
    // Create new tab with command and options
    createTab(command, targetMode, options);
    saveConsoleState();
  };
  ...
  /**
   * Force reconnect to a session by session ID
   * Creates new tab if session doesn't exist locally, reconnects if it does
   */
  window.forceReconnectSession = function(sessionId, targetMode = null, tabLabel = null) {
    // Show modal
    isMinimized = false;
    removeConsoleMinimizedIndicator();
    modal.classList.remove('hidden');
    ...
    if (existingTab) {
      ...
      existingTab.ws = null;
      switchToTab(existingTab.id);
      setTimeout(() => connectTab(existingTab, targetMode), 100);
    } else {
      // Create new tab with this session ID (will connect to existing server-side session)
      createTab(null, targetMode, { sessionId: sessionId, tabLabel: tabLabel || 'Session' });
    }
    saveConsoleState();
  };
