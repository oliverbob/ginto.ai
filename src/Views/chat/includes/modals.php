  ...
  <div class="absolute inset-0 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full transform transition-all scale-95 opacity-0" id="session-expired-modal-content">
      ...
      <div class="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-b-xl flex justify-end gap-3">
        ...
        <button onclick="closeSessionExpiredModal(); showLoginRequiredModal();" class="px-4 py-2 text-sm font-medium text-indigo-600 dark:text-indigo-400 bg-white dark:bg-gray-700 border border-indigo-300 dark:border-indigo-600 rounded-lg hover:bg-indigo-50 dark:hover:bg-gray-600 transition-colors inline-flex items-center gap-2">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="login-required-modal" class="fixed inset-0 z-[100] hidden">
  <!-- Backdrop with dynamic background -->
  <div id="login-modal-backdrop" class="absolute inset-0 bg-black/70 backdrop-blur-sm" onclick="closeLoginRequiredModal()"></div>
  
  <!-- Rain animation overlay (shown when hasUsers = true) -->
  <div id="rain-container" class="absolute inset-0 pointer-events-none overflow-hidden hidden"></div>
  
  <!-- Confetti celebration (shown when hasUsers = false) -->
  <div id="confetti-container" class="absolute inset-0 pointer-events-none overflow-hidden"></div>
  
  <!-- Modal Content -->
  <div class="absolute inset-0 flex items-center justify-center p-4">
    <div class="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-sm w-full transform transition-all">
      <!-- Close button -->
      <button onclick="closeLoginRequiredModal()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-white transition-colors z-10">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
        </svg>
      </button>
      
      <!-- Header with icon - CONGRATS variant (fresh install, no users) -->
      <div id="login-header-congrats" class="p-6 pb-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Congrats!</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">You can now sign in to test the models!</p>
          </div>
        </div>
      </div>
      
      <!-- Header with icon - SIGN IN variant (users exist) -->
      <div id="login-header-signin" class="p-6 pb-4 border-b border-gray-200 dark:border-gray-700 hidden">
        <div class="flex items-center gap-4">
          <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center shadow-lg">
            <!-- Umbrella icon -->
            <svg class="w-7 h-7 text-white" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2C6.48 2 2 6.48 2 12c0 .74.08 1.46.24 2.15.16.69.93 1.85 1.76 1.85h7v4c0 1.1.9 2 2 2s2-.9 2-2v-4h7c.83 0 1.6-1.16 1.76-1.85.16-.69.24-1.41.24-2.15 0-5.52-4.48-10-10-10zm-1 14H5.09C5.03 15.68 5 15.34 5 15c0-3.86 3.14-7 7-7s7 3.14 7 7c0 .34-.03.68-.09 1H13v-1h-2v1z"/>
            </svg>
          </div>
          <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Sign In</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Raining with opportunities!</p>
          </div>
        </div>
      </div>
      
      <!-- Login Form -->
      <form id="login-modal-form" class="p-6 space-y-4">
        <div id="login-modal-error" class="hidden p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl text-red-600 dark:text-red-400 text-sm"></div>
        
        <div>
          <label for="login-modal-identifier" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            Email or Username
          </label>
          <input type="text" id="login-modal-identifier" name="identifier" required autocomplete="username"
                 class="w-full px-4 py-3 rounded-xl border border-gray-600 bg-gray-800 text-white placeholder-gray-400 outline-none"
                 placeholder="Enter your email or username">
        </div>
        
        <div>
          <label for="login-modal-password" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            Password
          </label>
          <div class="relative">
            <input type="password" id="login-modal-password" name="password" required autocomplete="current-password"
                   class="w-full px-4 py-3 rounded-xl border border-gray-600 bg-gray-800 text-white placeholder-gray-400 outline-none pr-12"
                   placeholder="Enter your password">
            <button type="button" onclick="toggleLoginPasswordVisibility()" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
        ...
        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token ?? generateCsrfToken()) ?>">
      </form>
      
      <!-- Actions -->
      <div class="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-b-2xl flex items-center justify-between">
        <a href="/register" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 text-sm font-medium transition-colors">
          Create an account
        </a>
        <button type="submit" form="login-modal-form" id="login-modal-submit"
                class="px-6 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 rounded-xl shadow-lg shadow-indigo-500/25 transition-all flex items-center gap-2">
          <span>Continue</span>
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- Setup Required Modal (for fresh install without model) -->
<div id="setup-required-modal" class="fixed inset-0 z-[100] hidden">
  <!-- Backdrop -->
  <div class="absolute inset-0 bg-black/70 backdrop-blur-sm" onclick="closeSetupRequiredModal()"></div>
  <!-- Modal Content -->
  <div class="absolute inset-0 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-lg w-full transform transition-all">
      <!-- Header with icon -->
      <div class="p-6 pb-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Configure AI models to start chatting</p>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 space-y-4">
        <p class="text-gray-600 dark:text-gray-300">
          You need to configure <strong>AI models</strong> in Live Settings before you can start chatting with the <strong><em>fastest AI in the world</em></strong>. This only takes a minute!
        </p>
        ...
        <div class="space-y-3">
          <div class="flex items-start gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
            ...
            <div>
              <div class="font-medium text-gray-900 dark:text-white">Add API Keys</div>
              <div class="text-sm text-gray-600 dark:text-gray-400">Groq, Cerebras, or OpenRouter</div>
            </div>
          </div>
          ...
          <div class="flex items-start gap-3 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-xl">
            ...
            <div>
              <div class="font-medium text-gray-900 dark:text-white">Or Use Local Models</div>
              <div class="text-sm text-gray-600 dark:text-gray-400">Ollama, llama.cpp, or any OpenAI-compatible server</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-b-2xl flex justify-between items-center">
        <button onclick="closeSetupRequiredModal()" class="px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
          Later
        </button>
        <a href="/live" class="px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 rounded-xl shadow-lg shadow-green-500/25 transition-all inline-flex items-center gap-2">
          Go to Live Settings
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"/>
          </svg>
