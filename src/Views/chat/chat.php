...
<head>
  ...
  <style>
    ...
    /* Note: Sticky behavior is now handled by CSS on .code-block-header */
  ...
  (function() {
    ...
    function createTab(initialCommand) {
      ...
      const tab = {
        ...
        sessionId: 'session-' + Date.now() + '-' + Math.random().toString(36).substr(2, 9)  // Persistent session ID
      };
    ...
    function connectTab(tab) {
      ...
      // Include session ID for persistent sessions - allows reconnecting to same PTY
      ...
      tab.ws.addEventListener('open', function() {
        ...
        // Server sends session info on connect, no need for extra message
