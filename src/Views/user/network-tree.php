...
<body class="bg-gray-50 dark:bg-gray-900">
    ...
    // Admin flag - admins can search all users, not just their network
    const IS_ADMIN = <?= json_encode(!empty($isAdmin)) ?>;
    ...
    async function performSearch(query) {
        try {
            const cacheKey = (IS_ADMIN ? 'admin:' : '') + String(query).trim().toLowerCase();
            ...
            let matches;
            if (IS_ADMIN) {
                // Admin: search ALL users via remote API
                matches = await remoteSearch(query);
            } else {
                // Regular user: local search within their network tree only
                matches = localSearch(query);
            }
    ...
    // Remote search for admins - searches all users in database
    async function remoteSearch(query) {
        try {
            const url = `/api/user/network-search?q=${encodeURIComponent(query)}&csrf_token=${encodeURIComponent(CSRF_TOKEN)}`;
    ...
    function renderOrganizationalChart(container, data) {
        ...
        try {
            ...
            $__levels_for_js = $db->select('tier_plans', ['id', 'name', 'commission_rate_json', 'amount'], ['ORDER' => ['id' => 'ASC']]);
        } catch (Throwable $e) {
            $__levels_for_js = [];
        }
