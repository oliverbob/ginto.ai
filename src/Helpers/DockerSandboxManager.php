...
class DockerSandboxManager
{
    ...
    // Resource limits
    const DEFAULT_CPU_LIMIT = '1.0';  // Will be capped at available CPUs
     ...
     * 
     * @return float Number of CPUs (can be fractional for Docker)
     */
    public static function getAvailableCpus(): float
    {
        // Try to get CPU count from nproc
        $cpus = trim(shell_exec('nproc 2>/dev/null') ?? '');
        if ($cpus && is_numeric($cpus)) {
            return (float)$cpus;
        }
    ...
    public static function createSandbox(string $sandboxId, ?array $options = null): array
    {
        ...
        // Cap CPU limit at available CPUs (Docker requires --cpus <= available CPUs)
        $availableCpus = self::getAvailableCpus();
        if ((float)$cpuLimit > $availableCpus) {
            $cpuLimit = (string)$availableCpus;
        }
        
        // Build docker run command
        // Mount Docker socket to allow Docker-in-Docker for OpenWebUI etc.
        // Note: Disk quotas via --storage-opt require XFS with pquota, not available on ext4
