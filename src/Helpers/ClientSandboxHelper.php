...
class ClientSandboxHelper
{
        ...
        if ($db) {
            try {
                ...
                if ($userId) {
                    ...
                    // Visitors share a single sandbox with public_id='visitor'
                    $row = $db->get('client_sandboxes', ['sandbox_id'], [
                        'public_id' => 'visitor',
                        'ORDER' => ['id' => 'DESC']
                    ]);
                ...
                // Insert mapping with container info (NO directory creation!)
                $containerName = 'ginto-sandbox-' . $candidate;
                $data = [
                ...
                if ($userId) {
                    ...
                    // Visitors share a single sandbox - use 'visitor' as the public_id
                    $data['public_id'] = 'visitor';
                }
    ...
    public static function getSandboxIdIfExists($db = null, $session = null, bool $validate = false): ?string
    {
        ...
        // Priority: For logged-in users, always look up their sandbox from DB first
        // This ensures a user sees THEIR sandbox, not a leftover visitor sandbox in session
        if ($db && $userId) {
            try {
        ...
        // Fall back to session sandbox_id (for visitors or if DB lookup failed)
        if (!$sandboxId && !empty($session['sandbox_id'])) {
            $sandboxId = $session['sandbox_id'];
        }
        
        // Final fallback: look up shared visitor sandbox (public_id='visitor')
        if (!$sandboxId && $db && $publicId && !$userId) {
            try {
                $row = $db->get('client_sandboxes', ['sandbox_id'], ['public_id' => 'visitor']);
                if (!empty($row['sandbox_id'])) {
                    $sandboxId = $row['sandbox_id'];
                }
            } catch (\Throwable $_) {}
     ...
     * Also cleans up stale DB entries when container is missing.
     * 
    ...
    public static function validateSandboxExists(string $sandboxId, $db = null): bool
    {
        ...
        if ($db) {
            try {
                $dbExists = $db->has('client_sandboxes', ['sandbox_id' => $sandboxId]);
                if (!$dbExists) {
