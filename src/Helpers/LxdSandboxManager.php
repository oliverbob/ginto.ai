...
class LxdSandboxManager
{
    ...
    // Template container name - the pre-configured ginto-sandbox template
    // This container contains: PHP 8.2 (with extensions), Composer, Node.js 20, npm, 
    // Python 3.12, Git, MySQL client, SQLite, Caddy, Vim, Nano
    // Services (php-fpm82, caddy) auto-start via OpenRC
    // User sandboxes are created by copying this container
    const BASE_CONTAINER = 'ginto-sandbox';
    ...
    public static function checkLxcAvailability(): array
    {
        ...
        // Check if template container exists (ginto-sandbox)
        exec(self::LXC_CMD . ' info ' . self::BASE_CONTAINER . ' 2>/dev/null', $containerOutput, $containerCode);
        if ($containerCode !== 0) {
            return [
                'available' => false,
    ...
    public static function ensureSandboxAccessible(string $userId): bool
    {
        ...
        // Ensure container exists (uses fast API if available)
        ...
        // Ensure container is running (uses fast API if available)
        if (!self::sandboxRunning($userId)) {
            // Try fast API start first
            if (LxdApiClient::isAvailable()) {
                LxdApiClient::startInstance($name);
            } else {
        ...
        // Get actual IP from container (uses fast API if available)
     ...
     * 
     * Uses LXD REST API via Unix socket for speed (~5ms vs ~200ms CLI).
    ...
    public static function sandboxExists(string $userId): bool
    {
        ...
        // Try fast API first (5-20ms vs 100-300ms for CLI)
        if (LxdApiClient::isAvailable()) {
            return LxdApiClient::instanceExists($name);
        }
        
        // Fallback to CLI if API not available
        $code = 0;
        exec(self::LXC_CMD . " info " . escapeshellarg($name) . " 2>/dev/null", $output, $code);
    ...
    public static function sandboxRunning(string $userId): bool
    {
        ...
        // Try fast API first (5-20ms vs 100-300ms for CLI)
        if (LxdApiClient::isAvailable()) {
            return LxdApiClient::instanceRunning($name);
        }
    ...
    public static function queryContainerIp(string $userId): ?string
    {
        ...
        // Try fast API first (5-20ms vs 100-300ms for CLI)
        if (LxdApiClient::isAvailable()) {
            return LxdApiClient::getInstanceIp($name);
        }
     ...
     * 
     * Uses LXD REST API to initiate container copy in background.
     * Returns immediately with an operation ID for polling.
     * 
     * Frontend should poll /api/sandbox/creation-status to check progress.
     * 
     * @param string $userId User identifier
     * @return array ['success' => bool, 'status' => string, 'operation' => string|null, 'ip' => string|null, 'sandboxId' => string, 'error' => string|null]
     */
    public static function createSandboxAsync(string $userId): array
    {
        $name = self::containerName($userId);
        $ip = self::sandboxToIp($userId);
        ...
        if (self::sandboxExists($userId)) {
            // Start if not running (async)
            if (!self::sandboxRunning($userId)) {
                $startResult = LxdApiClient::startInstanceAsync($name);
                if ($startResult['success'] && $startResult['operation']) {
                    return [
                        'success' => true,
                        'status' => 'starting',
                        ...
                        'sandboxId' => $name,
                        'error' => null,
                        'message' => 'Sandbox exists, starting...',
                    ];
                }
            }
        ...
        if (!LxdApiClient::isAvailable()) {
            // Fallback to blocking exec method
            return self::createSandbox($userId);
        }
        
        // Initiate async copy from template
        $copyResult = LxdApiClient::copyInstanceAsync(self::BASE_CONTAINER, $name);
        ...
        if (!$copyResult['success']) {
            return [
                ...
                'ip' => null,
                'sandboxId' => $name,
                'error' => 'Failed to initiate copy: ' . ($copyResult['error'] ?? 'Unknown error'),
            ];
        }
        
        // Store operation ID in Redis for polling
        $redis = self::getRedis();
        if ($redis) {
            try {
                $redis->setex(self::REDIS_PREFIX . $userId . ':create_op', 600, $copyResult['operation']);
                $redis->setex(self::REDIS_PREFIX . $userId . ':create_status', 600, 'copying');
            } catch (\Throwable $e) {
                // Redis is optional
            }
        }
        ...
        return [
            'success' => true,
            'status' => 'copying',
            'operation' => $copyResult['operation'],
            'ip' => $ip,
            'sandboxId' => $name,
            'error' => null,
            'message' => 'Sandbox creation started in background',
        ];
    }
    
    /**
     * Check sandbox creation status (for polling)
     * 
     * @param string $userId User identifier
     * @return array ['status' => string, 'ready' => bool, 'ip' => string|null, 'error' => string|null]
     */
    public static function getCreationStatus(string $userId): array
    {
        $name = self::containerName($userId);
        $ip = self::sandboxToIp($userId);
        
        // If sandbox exists and running, it's ready
        if (self::sandboxExists($userId)) {
            if (self::sandboxRunning($userId)) {
                return [
                    'status' => 'ready',
        ...
        // Check Redis for pending operation
        $redis = self::getRedis();
        if ($redis) {
            try {
                $opPath = $redis->get(self::REDIS_PREFIX . $userId . ':create_op');
                if ($opPath) {
                    // Poll LXD operation status
                    $opStatus = LxdApiClient::getOperation($opPath);
                    ...
                    if (!$opStatus) {
                        return [
                            ...
                            'ip' => null,
                            'sandboxId' => $name,
                            'error' => 'Could not get operation status',
                        ];
                    }
                    
                    $status = strtolower($opStatus['status'] ?? 'unknown');
                    
                    if ($status === 'success') {
                        // Copy completed, now configure and start
                        $redis->del(self::REDIS_PREFIX . $userId . ':create_op');
                        $redis->setex(self::REDIS_PREFIX . $userId . ':create_status', 600, 'configuring');
                        
                        // Configure and start in background (quick operations)
                        self::configureAndStartSandbox($userId);
                        ...
                        return [
                            ...
                            'ip' => null,
                            'sandboxId' => $name,
                            'progress' => $opStatus['metadata']['progress'] ?? null,
                            'error' => null,
                        ];
                    } elseif ($status === 'failure' || $status === 'cancelled') {
                        $redis->del(self::REDIS_PREFIX . $userId . ':create_op');
                        $redis->del(self::REDIS_PREFIX . $userId . ':create_status');
                        ...
                        return [
                            ...
                            'ip' => null,
                            'sandboxId' => $name,
                            'error' => $opStatus['err'] ?? 'Operation failed',
                        ];
                    }
                }
            } catch (\Throwable $e) {
        ...
        // No sandbox, no pending operation
        return [
            'status' => 'not_created',
            'ready' => false,
            'ip' => null,
    ...
    /**
     * Configure and start a newly copied sandbox container
     * 
     * Called after async copy completes. Applies security settings, 
     * static IP, and starts services.
     * 
     * @param string $userId User identifier
     * @return bool Success
     */
    public static function configureAndStartSandbox(string $userId): bool
    {
        $name = self::containerName($userId);
        $ip = self::sandboxToIp($userId);
        ...
        $startResult = LxdApiClient::startInstanceAsync($name);
        if ($startResult['operation']) {
            // Wait for start to complete (usually quick, ~1-2s)
            LxdApiClient::waitOperation($startResult['operation'], 30);
        }
        
        // Apply security settings and static IP (using exec for now, could be optimized later)
        $networkMode = self::getNetworkMode();
        
        if ($networkMode === 'macvlan' || $networkMode === 'ipvlan') {
            // MACVLAN/IPVLAN mode configuration
            $parentIface = self::getNetworkInterface();
            $nicType = ($networkMode === 'macvlan') ? 'macvlan' : 'ipvlan';
            
            exec(self::LXC_CMD . " config device remove $name eth0 2>/dev/null");
            exec(self::LXC_CMD . " config device add $name eth0 nic nictype=$nicType parent=$parentIface 2>&1");
        ...
        // Security hardening (same as sync version)
        ...
        exec(self::LXC_CMD . " config set $name limits.cpu 2 2>&1");
        exec(self::LXC_CMD . " config set $name limits.memory 1GB 2>&1");
        exec(self::LXC_CMD . " config set $name limits.processes 200 2>&1");
    ...
    public static function createSandbox(string $userId, array $options = []): array
    {
        ...
        if ($code !== 0) {
            return [
                ...
                'ip' => null,
                'sandboxId' => $name,
                'error' => 'Failed to copy container: ' . implode("\n", $output)
            ];
        }
        
        // Start the new container
        $startOutput = [];
        exec(self::LXC_CMD . " start $name < /dev/null 2>&1", $startOutput, $startCode);
        ...
        if ($startCode !== 0) {
            return [
                ...
                'ip' => null,
                'sandboxId' => $name,
                'error' => 'Failed to start container: ' . implode("\n", $startOutput)
            ];
        }
        ...
        if ($networkMode === 'macvlan' || $networkMode === 'ipvlan') {
            ...
            $caddyStartScript = "#!/bin/sh\ncd /root && caddy start --config /etc/caddy/Caddyfile --adapter caddyfile 2>/dev/null &\n";
        ...
        // =======================================================================
        // XFCE PANEL CONFIGURATION - Full desktop with taskbar
        // =======================================================================
        self::ensureXfcePanel($name);
    ...
    /**
     * Setup XFCE panel configuration for desktop with taskbar
     * 
     * Creates the panel config with:
     * - Top panel: Applications menu, spacer, clock/date, pager (workspace switcher)
     * - Bottom dock: Quick launch icons for common apps
     * This is applied to each new sandbox since the base template doesn't include it.
     * Also called when starting VNC on existing sandboxes to ensure panel config exists.
     */
    public static function ensureXfcePanel(string $containerName): void
    {
        // Always apply panel config to ensure correct layout
        // (Base image may have different config)
        
        // XFCE panel configuration XML - Top bar + Bottom dock layout
        $panelXml = '<?xml version="1.0" encoding="UTF-8"?>
<channel name="xfce4-panel" version="1.0">
  ...
  <property name="plugins" type="empty">
    ...
    <property name="plugin-16" type="string" value="showdesktop"/>
        ...
        // Create config directory and write panel config
        $escapedXml = base64_encode($panelXml);
        exec(self::LXC_CMD . " exec $containerName -- sh -c 'mkdir -p /root/.config/xfce4/xfconf/xfce-perchannel-xml && echo $escapedXml | base64 -d > /root/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml' 2>&1");
        
        // Also set Chromium as default browser
        exec(self::LXC_CMD . " exec $containerName -- sh -c 'mkdir -p /root/.config/xfce4 && echo \"WebBrowser=chromium\" > /root/.config/xfce4/helpers.rc' 2>&1");
        
        // Restart the container to apply config cleanly (XFCE will start fresh via OpenRC)
        exec(self::LXC_CMD . " restart $containerName < /dev/null 2>&1");
        sleep(5); // Wait for container and services to come up
    }
     ...
     */
    public static function execInSandbox(string $userId, string $command, string $cwd = '/root', int $timeout = 30): array
    {
        $name = self::containerName($userId);
     ...
     * 
     * Uses LXD REST API via Unix socket for speed where available.
     * 
     ...
     * 
     * @param string $userId User/sandbox identifier
     * @param string $remotePath Path inside container (default: /root)
     * @param int $maxDepth Maximum recursion depth
     * @return array ['success' => bool, 'tree' => array, 'error' => string|null]
     */
    public static function listFiles(string $userId, string $remotePath = '/root', int $maxDepth = 5): array
    {
        $name = self::containerName($userId);
     ...
     * 
     * @param string $userId User/sandbox identifier
     * @param string $filePath Path inside container relative to /root
     ...
     * 
     * @param string $userId User/sandbox identifier
     * @param string $path Path inside container relative to /root
     ...
     * 
     * @param string $userId User/sandbox identifier
     * @param string $oldPath Old path relative to /root
     * @param string $newPath New path relative to /root
     * @return array ['success' => bool, 'error' => string|null]
     */
    public static function renameItem(string $userId, string $oldPath, string $newPath): array
    {
        ...
        $oldFullPath = '/root/' . ltrim($oldPath, '/');
     ...
     * 
     * @param string $userId User/sandbox identifier
     * @param string $sourcePath Source path relative to /root
     * @param string $destPath Destination path relative to /root
     * @return array ['success' => bool, 'error' => string|null]
     */
    public static function copyItem(string $userId, string $sourcePath, string $destPath): array
    {
        ...
        $sourceFullPath = '/root/' . ltrim($sourcePath, '/');
        $destFullPath = '/root/' . ltrim($destPath, '/');
