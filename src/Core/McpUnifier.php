...
class McpUnifier
{
    ...
    private function discoverHandlersViaAttributes(): array
    {
        ...
        foreach (scandir($hd) as $f) {
            ...
            // Check both App\ and Ginto\ namespaces (without triggering autoload to avoid redeclare errors)
            $fqcn = '\\App\\Handlers\\' . $className;
            $altFqcn = '\\Ginto\\Handlers\\' . $className;
            if (!class_exists($fqcn, false) && !class_exists($altFqcn, false)) continue;
            // Use whichever class actually exists
            $actualFqcn = class_exists($fqcn, false) ? $fqcn : $altFqcn;
            try {
                $rc = new \ReflectionClass($actualFqcn);
                foreach ($rc->getMethods() as $m) {
                    ...
                    foreach ($all as $attr) {
                        ...
                        $tools[] = [
                            ...
                            'source' => 'handlers',
                            'mcp' => 'app',
                            'handler' => $actualFqcn . '::' . $m->getName(),
                            'raw' => ['class' => $actualFqcn, 'method' => $m->getName(), 'attr' => $args],
                        ];
                    }
                }
     ...
     * This properly extracts parameter schemas from handler method signatures.
     * 
     * @param array $filterNames Optional list of tool names to include (for filtering)
     * @return array Tools formatted for OpenAI/Groq/Cerebras tools parameter
     */
    public function getToolsAsOpenAI(array $filterNames = []): array
    {
        $root = defined('ROOT_PATH') ? ROOT_PATH : dirname(__DIR__, 2);
        $hd = $root . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'Handlers';
        if (!is_dir($hd)) return [];
            ...
            try {
                ...
                foreach ($rc->getMethods() as $method) {
                    foreach ($method->getAttributes() as $attr) {
                        if (stripos($attr->getName(), 'McpTool') === false) continue;
                        ...
                        // Filter if specified
                        if (!empty($filterNames) && !in_array($name, $filterNames, true)) continue;
                        
                        $desc = $args['description'] ?? 'Tool: ' . $name;
                        ...
                        foreach ($method->getParameters() as $param) {
                            ...
                            if ($paramName === 'sandbox_id') continue;
                            ...
                            if (!$param->isOptional() && (!$paramType || !$paramType->allowsNull())) {
                                $required[] = $paramName;
                            }
                        }
