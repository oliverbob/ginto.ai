 ...
 * ======================
 * When running in Docker (DOCKER_MODE=true or INSTALL_MODE=docker in .env),
 * URLs default to host.docker.internal instead of 127.0.0.1 to reach
 * llama.cpp servers running on the host machine.
 * 
 * CONFIGURATION (.env):
 * =====================
 ...
 * DOCKER_MODE=true                             # Set automatically for Docker installs
 * 
...
class LocalLLMConfig
{
    ...
    {
        // Detect Docker mode using centralized helper
        $this->isDockerMode = ProviderRegistry::isDockerMode();
        
        // Use host.docker.internal in Docker mode to reach host machine's llama servers
        $defaultHost = ProviderRegistry::getHostAddress();
        
        $this->reasoningUrl = $this->getEnv('LOCAL_LLM_URL', "http://{$defaultHost}:8034/v1");
        $this->reasoningModel = $this->getEnv('LOCAL_LLM_MODEL', 'local');
        $this->visionUrl = $this->getEnv('VISION_MODEL_URL', "http://{$defaultHost}:8033/v1");
    ...
    public function isDockerMode(): bool
    {
