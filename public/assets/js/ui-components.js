...
const GintoUI = {
    ...
    autocomplete(container, options = {}) {
        ...
        dropdown.className = 'ginto-autocomplete-dropdown bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl max-h-64 overflow-y-auto hidden backdrop-blur-sm';
        // Render dropdown as a top-level element to avoid stacking-context issues
        ...
        const showDropdown = () => {
            // Position dropdown under input using viewport coordinates so it appears above fixed elements
        ...
        // Reposition on window changes to keep dropdown aligned with input
        const reposition = () => { if (!isOpen) return; const rect = input.getBoundingClientRect(); dropdown.style.left = Math.max(8, rect.left) + 'px'; dropdown.style.top = (rect.bottom + 8) + 'px'; dropdown.style.width = rect.width + 'px'; };
