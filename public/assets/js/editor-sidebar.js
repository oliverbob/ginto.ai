...
(function(win){
  ...
  // Drag overlay utility: creates a transparent full-screen overlay during drag
  // to prevent iframes and other elements from stealing pointer events.
  // The overlay captures all pointer/mouse/touch events so they don't get lost to iframes.
  var dragOverlay = null;
  var overlayMoveHandler = null;
  var overlayEndHandler = null;

  function showDragOverlay(moveHandler, endHandler) {
    if (dragOverlay) return;
    overlayMoveHandler = moveHandler;
    overlayEndHandler = endHandler;
    dragOverlay = document.createElement('div');
    dragOverlay.id = 'ginto-resizer-drag-overlay';
    // Use a very subtle semi-transparent background to ensure the overlay is "hittable"
    // Some browsers may optimize away completely transparent overlays for hit testing
    dragOverlay.style.cssText = 'position:fixed;inset:0;z-index:999999;cursor:col-resize;background:rgba(0,0,0,0.001);touch-action:none;-webkit-user-select:none;user-select:none;';
    
    // Attach event listeners directly to the overlay - this ensures events are captured
    // even when the pointer would otherwise be over an iframe
    if (moveHandler) {
      dragOverlay.addEventListener('pointermove', moveHandler);
      dragOverlay.addEventListener('mousemove', moveHandler);
      dragOverlay.addEventListener('touchmove', moveHandler, { passive: false });
    }
    if (endHandler) {
      dragOverlay.addEventListener('pointerup', endHandler);
      dragOverlay.addEventListener('mouseup', endHandler);
      dragOverlay.addEventListener('touchend', endHandler);
      // Also handle pointer leaving the window
      dragOverlay.addEventListener('pointercancel', endHandler);
      dragOverlay.addEventListener('touchcancel', endHandler);
    }
  ...
  function hideDragOverlay() {
    if (dragOverlay) {
      ...
      if (overlayMoveHandler) {
        try { dragOverlay.removeEventListener('pointermove', overlayMoveHandler); } catch(e){}
        try { dragOverlay.removeEventListener('mousemove', overlayMoveHandler); } catch(e){}
        try { dragOverlay.removeEventListener('touchmove', overlayMoveHandler); } catch(e){}
      }
      if (overlayEndHandler) {
        try { dragOverlay.removeEventListener('pointerup', overlayEndHandler); } catch(e){}
        try { dragOverlay.removeEventListener('mouseup', overlayEndHandler); } catch(e){}
        try { dragOverlay.removeEventListener('touchend', overlayEndHandler); } catch(e){}
        try { dragOverlay.removeEventListener('pointercancel', overlayEndHandler); } catch(e){}
        try { dragOverlay.removeEventListener('touchcancel', overlayEndHandler); } catch(e){}
      }
    ...
    try {
      ...
      var sidebar = registry.container;
        ...
        function stopDrag() {
          ...
          try { document.body.classList.remove('resizer-dragging'); } catch(e){}
          try { hideDragOverlay(); } catch(e){}
          // persist width
        ...
        if (usingPointerEvents) {
          resizer.addEventListener('pointerdown', function(ev){
            try {
              ...
              try { document.body.classList.add('resizer-dragging'); } catch(e){}
              try { showDragOverlay(onPointerMove, stopDrag); } catch(e){}
          ...
          if (rightPane && parent && !parent.querySelector('.editor-resizer-right')) {
            ...
            if (usingPointerEventsR) {
              rResizer.addEventListener('pointerdown', function(ev){
                try { if (ev.isPrimary === false) return; draggingR = true; startXR = ev.clientX; startWidthR = Math.round(rightPane.getBoundingClientRect().width); activePointerIdR = ev.pointerId; try { if (rResizer.setPointerCapture) rResizer.setPointerCapture(activePointerIdR); } catch(e){} rResizer.classList.add('resizing'); rightPane.classList.add('resizing'); try { document.body.classList.add('resizer-dragging'); } catch(e){} try { showDragOverlay(onPointerMoveR, stopDragR); } catch(e){} document.addEventListener('pointermove', onPointerMoveR); document.addEventListener('pointerup', stopDragR); ev.preventDefault(); if (window.__ginto_preview_log && typeof window.__ginto_preview_log === 'function') { window.__ginto_preview_log('parent-resizer-right', { ev: 'pointerdown', x: ev.clientX, pointerId: ev.pointerId }); } else { window.postMessage({ __ginto_preview: true, type: 'parent-resizer-right', msg: JSON.stringify({ ev: 'pointerdown', x: ev.clientX, pointerId: ev.pointerId }) }, window.location.origin); } } catch(e){}
              });
            } else {
              rResizer.addEventListener('mousedown', function(ev){ draggingR = true; startXR = ev.clientX; startWidthR = Math.round(rightPane.getBoundingClientRect().width); rResizer.classList.add('resizing'); rightPane.classList.add('resizing'); try { document.body.classList.add('resizer-dragging'); } catch(e){} try { showDragOverlay(onPointerMoveR, stopDragR); } catch(e){} document.addEventListener('mousemove', onPointerMoveR); document.addEventListener('mouseup', stopDragR); ev.preventDefault(); try { if (window.__ginto_preview_log && typeof window.__ginto_preview_log === 'function') { window.__ginto_preview_log('parent-resizer-right', { ev: 'pointerdown', x: ev.clientX }); } else { window.postMessage({ __ginto_preview: true, type: 'parent-resizer-right', msg: JSON.stringify({ ev: 'pointerdown', x: ev.clientX }) }, window.location.origin); } } catch(e){} });
            }

            rResizer.addEventListener('touchstart', function(ev){ draggingR = true; startXR = ev.touches[0].clientX; startWidthR = Math.round(rightPane.getBoundingClientRect().width); rResizer.classList.add('resizing'); rightPane.classList.add('resizing'); try { document.body.classList.add('resizer-dragging'); } catch(e){} try { showDragOverlay(onPointerMoveR, stopDragR); } catch(e){} document.addEventListener('touchmove', onPointerMoveR, { passive: false }); document.addEventListener('touchend', stopDragR); ev.preventDefault(); try { var t = ev.touches && ev.touches[0]; if (window.__ginto_preview_log && typeof window.__ginto_preview_log === 'function') { window.__ginto_preview_log('parent-resizer-right', { ev: 'touchstart', x: t && t.clientX || 0, y: t && t.clientY || 0 }); } else { window.postMessage({ __ginto_preview: true, type: 'parent-resizer-right', msg: JSON.stringify({ ev: 'touchstart', x: t && t.clientX || 0, y: t && t.clientY || 0 }) }, window.location.origin); } } catch(e){} }, { passive: false });
      ...
      try {
        ...
        if (existingR && existingPane && !existingR.dataset.gintoAttached) {
          ...
          (function(rRes, pane){
            ...
            function stopDragR() { if (!draggingR) return; draggingR = false; rRes.classList.remove('resizing'); pane.classList.remove('resizing'); try { document.body.classList.remove('resizer-dragging'); } catch(e){} try { hideDragOverlay(); } catch(e){} try { localStorage.setItem('ginto.editor.rightpane.width', Math.round(pane.getBoundingClientRect().width)); } catch(e){}; try { document.removeEventListener('mousemove', onPointerMoveR); document.removeEventListener('mouseup', stopDragR); document.removeEventListener('touchmove', onPointerMoveR); document.removeEventListener('touchend', stopDragR); document.removeEventListener('pointermove', onPointerMoveR); document.removeEventListener('pointerup', stopDragR); } catch(e){} }
            var usingPointerEventsR = !!window.PointerEvent; var activePointerIdR = null;
            if (usingPointerEventsR) {
              rRes.addEventListener('pointerdown', function(ev){ try { if (ev.isPrimary === false) return; draggingR = true; startXR = ev.clientX; startWidthR = Math.round(pane.getBoundingClientRect().width); activePointerIdR = ev.pointerId; try { if (rRes.setPointerCapture) rRes.setPointerCapture(activePointerIdR); } catch(e){} rRes.classList.add('resizing'); pane.classList.add('resizing'); try { document.body.classList.add('resizer-dragging'); } catch(e){} try { showDragOverlay(); } catch(e){} document.addEventListener('pointermove', onPointerMoveR); document.addEventListener('pointerup', stopDragR); ev.preventDefault(); } catch(e){} });
            } else {
              rRes.addEventListener('mousedown', function(ev){ draggingR = true; startXR = ev.clientX; startWidthR = Math.round(pane.getBoundingClientRect().width); rRes.classList.add('resizing'); pane.classList.add('resizing'); try { document.body.classList.add('resizer-dragging'); } catch(e){} try { showDragOverlay(); } catch(e){} document.addEventListener('mousemove', onPointerMoveR); document.addEventListener('mouseup', stopDragR); ev.preventDefault(); });
            }
            rRes.addEventListener('touchstart', function(ev){ draggingR = true; startXR = ev.touches[0].clientX; startWidthR = Math.round(pane.getBoundingClientRect().width); rRes.classList.add('resizing'); pane.classList.add('resizing'); try { document.body.classList.add('resizer-dragging'); } catch(e){} try { showDragOverlay(); } catch(e){} document.addEventListener('touchmove', onPointerMoveR, { passive:false }); document.addEventListener('touchend', stopDragR); ev.preventDefault(); }, { passive:false });
