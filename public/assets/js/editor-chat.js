  ...
  function formatToolResult(toolName, result) {
    ...
    if (toolName === 'sandbox_read_file' && data?.content !== undefined) {
      ...
      if (isBinary && ['png','jpg','jpeg','gif','webp','svg','pdf'].includes(ext)) {
        ...
        return '<div class="tool-result" style="margin:8px 0;">' +
          '<p style="color:#d1d5db;margin-bottom:8px;">Preview of <code style="background:#374151;padding:2px 6px;border-radius:4px;">' + escapeHtml(path) + '</code>:</p>' +
          '<div style="text-align:center;margin-top:8px;"><img src="' + dataUrl + '" alt="' + escapeHtml(path) + '" style="max-width:100%;max-height:480px;border-radius:6px;" /></div>' +
        '</div>';
      }

      if (isBinary && ['docx','doc','odt','rtf'].includes(ext)) {
        var sandboxId = data.sandbox_id || window.editorConfig?.sandboxId || '';
        ...
        return '<div class="tool-result" style="margin:8px 0;">' +
          ...
          '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">' +
            '<a href="' + clientUrl + '" target="_blank" rel="noopener" style="padding:8px 12px;background:#111827;color:#fff;border-radius:6px;text-decoration:none;">Open / Download</a>' +
      ...
      var safeContent = encoding === 'base64' ? '(binary file - open to download)' : content;
      return '<div class="tool-result" style="margin:8px 0;">' +
    ...
    if (toolName === 'sandbox_create_document' && data?.success) {
      ...
      var format = data.format || 'Document';
      var formatKey = data.format_key || 'txt';
      var bytes = data.bytes_written || 0;
      var url = data.url || '/clients/' + (data.path || '').replace(/^\//, '');
      var openHint = data.open_hint || '';
      
      // Format-specific styling including PDF and DOCX
      var formatStyles = {
        pdf: { icon: '📕', bg: 'rgba(239,68,68,0.1)', border: 'rgba(239,68,68,0.3)', text: '#fca5a5', btn: 'rgba(239,68,68,0.2)' },
        docx: { icon: '📘', bg: 'rgba(59,130,246,0.1)', border: 'rgba(59,130,246,0.3)', text: '#93c5fd', btn: 'rgba(59,130,246,0.2)' },
        odt: { icon: '📙', bg: 'rgba(249,115,22,0.1)', border: 'rgba(249,115,22,0.3)', text: '#fdba74', btn: 'rgba(249,115,22,0.2)' },
        html: { icon: '🌐', bg: 'rgba(6,182,212,0.1)', border: 'rgba(6,182,212,0.3)', text: '#67e8f9', btn: 'rgba(6,182,212,0.2)' },
        rtf: { icon: '📄', bg: 'rgba(168,85,247,0.1)', border: 'rgba(168,85,247,0.3)', text: '#c4b5fd', btn: 'rgba(168,85,247,0.2)' },
        md: { icon: '📝', bg: 'rgba(31,41,55,0.5)', border: 'rgba(107,114,128,0.3)', text: '#d1d5db', btn: 'rgba(107,114,128,0.2)' },
        txt: { icon: '📃', bg: 'rgba(31,41,55,0.5)', border: 'rgba(107,114,128,0.3)', text: '#d1d5db', btn: 'rgba(107,114,128,0.2)' }
      };
      var style = formatStyles[formatKey] || formatStyles.txt;
      ...
      return '<div class="tool-result" style="padding:16px;background:' + style.bg + ';border:1px solid ' + style.border + ';border-radius:8px;margin:8px 0;">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:8px;">' +
        '<div style="display:flex;align-items:center;gap:12px;color:' + style.text + ';">' +
        '<span style="font-size:28px;">' + style.icon + '</span>' +
        '<div><div style="font-weight:600;font-size:15px;">' + escapeHtml(filename) + '</div>' +
        '<div style="font-size:12px;opacity:0.75;">' + escapeHtml(format) + (bytes > 0 ? ' • ' + formatSize(bytes) : '') + '</div></div></div>' +
        '<a href="' + escapeHtml(url) + '" target="_blank" download="' + escapeHtml(filename) + '" style="display:flex;align-items:center;gap:6px;padding:10px 16px;background:' + style.btn + ';border-radius:6px;color:' + style.text + ';text-decoration:none;font-size:13px;font-weight:600;">' +
        ...
        'Download</a></div>' +
        (openHint ? '<div style="font-size:11px;color:' + style.text + ';opacity:0.6;margin-top:8px;">💡 ' + escapeHtml(openHint) + '</div>' : '') +
        '</div>';
    }
    ...
    if (toolName === 'sandbox_list_document_formats' && data?.formats) {
      ...
      return '<div class="tool-result" style="padding:16px;background:rgba(31,41,55,0.5);border:1px solid #374151;border-radius:8px;margin:8px 0;">' +
        '<div style="font-weight:500;color:#e5e7eb;margin-bottom:12px;">📄 Available Document Formats</div>' +
        '<div style="display:grid;gap:8px;">' +
        formats.map(function(f) {
          return '<div style="display:flex;align-items:center;gap:12px;padding:8px;background:rgba(55,65,81,0.3);border-radius:4px;">' +
            '<code style="background:#4b5563;padding:4px 8px;border-radius:4px;font-size:12px;color:#93c5fd;">' + escapeHtml(f.format) + '</code>' +
            '<span style="color:#d1d5db;">' + escapeHtml(f.name) + '</span>' +
            '<span style="color:#6b7280;font-size:12px;">' + escapeHtml(f.extension) + '</span></div>';
        }).join('') +
        '</div>' +
        (data.tip ? '<div style="font-size:11px;color:#9ca3af;margin-top:12px;">💡 ' + escapeHtml(data.tip) + '</div>' : '') +
        '</div>';
    }
            ...
            while (true) {
              ...
              while (buf.indexOf('\n\n') !== -1) {
                ...
                try {
                  ...
                  // Handle server-side agentic tool execution events (from /chat endpoint)
                  // These are structured tool_use and tool_result events
                  if (obj && obj.tool_use) {
                    // Server is starting to execute a tool - show spinner
                    clearThinking();
                    var toolNote = document.createElement('div');
                    toolNote.className = 'assistant-message bot tool-exec-' + (obj.tool_name || 'unknown').replace(/[^a-zA-Z0-9]/g, '_');
                    toolNote.innerHTML = '<div class="bubble" style="padding:12px;background:rgba(31,41,55,0.5);border:1px solid rgba(75,85,99,0.5);border-radius:8px;">' +
                      ...
                      '</svg>' +
                      '<span>Executing <code style="background:#374151;padding:2px 6px;border-radius:4px;">' + escapeHtml(obj.tool_name || 'tool') + '</code>...</span>' +
                      '</div></div>';
                    body.appendChild(toolNote);
                    if (isScrolledToBottom(body, 48)) scrollToBottom(body);
                    continue;
                  }
                  
                  if (obj && obj.tool_result) {
                    // Server finished executing a tool - replace spinner with result
                    var toolClass = 'tool-exec-' + (obj.tool_name || 'unknown').replace(/[^a-zA-Z0-9]/g, '_');
                    var existingNote = body.querySelector('.' + toolClass);
                    if (existingNote) {
                      existingNote.className = 'assistant-message bot tool-result-formatted';
                      existingNote.innerHTML = '<div class="bubble">' + formatToolResult(obj.tool_name, obj) + '</div>';
                    } else {
                      // No spinner found, just append result
                      var resultNote = document.createElement('div');
                      resultNote.className = 'assistant-message bot tool-result-formatted';
                      resultNote.innerHTML = '<div class="bubble">' + formatToolResult(obj.tool_name, obj) + '</div>';
                      body.appendChild(resultNote);
                    }
                    if (isScrolledToBottom(body, 48)) scrollToBottom(body);
                    
                    // Create checkpoint for file write operations that include original content
                    var fileWriteOps = ['sandbox_write_file', 'write_file'];
                    var checkpointCreatedForResult = null;
                    if (fileWriteOps.indexOf(obj.tool_name) !== -1 && obj.result && obj.result.path) {
                      var toolResultPath = obj.result.path;
                      var originalContentFromResult = obj.result.original_content;
                      var newContentFromArgs = obj.tool_args && obj.tool_args.content ? obj.tool_args.content : null;
                      var isNewFile = obj.result.is_new_file;
                      
                      // Create checkpoint if we have the content info
                      // For new files, originalContent is null (we use empty string)
                      // For existing files, we use the original_content from the result
                      if (newContentFromArgs !== null) {
                        var origContent = isNewFile ? '' : (originalContentFromResult || '');
                        checkpointCreatedForResult = createAiCheckpoint(
                          toolResultPath,
                          origContent,
                          newContentFromArgs,
                          obj.tool_name + ' via AI'
                        );
                        console.log('[tool_result] Created checkpoint for:', toolResultPath, 'isNewFile:', isNewFile);
                        
                        // Add restore button
                        if (checkpointCreatedForResult && body) {
                          var restoreBtn = createRestoreCheckpointButton(checkpointCreatedForResult.id, toolResultPath);
                          body.appendChild(restoreBtn);
                          if (isScrolledToBottom(body, 48)) scrollToBottom(body);
                        }
                        
                        // Reload the file if it's currently open in the editor
                        var currentFile = window.currentFile || '';
                        var normalizedToolPath = normalizeRepoPath(toolResultPath);
                        var normalizedCurrent = normalizeRepoPath(currentFile);
                        if (normalizedToolPath && normalizedCurrent && normalizedToolPath === normalizedCurrent) {
                          setTimeout(function() {
                            if (window.currentEncoded && editor && typeof editor.loadFile === 'function') {
                              editor.loadFile(window.currentEncoded, currentFile).then(function() {
                                console.log('[tool_result] Reloaded file in editor:', currentFile);
                              }).catch(function(err) {
                                console.error('[tool_result] Failed to reload file:', err);
                              });
                              if (editor.showToast) {
                                editor.showToast('File updated by AI');
                              }
                    ...
                    // Refresh file tree if a file operation was performed
                    var fileOps = ['sandbox_write_file', 'sandbox_create_document', 'sandbox_delete_file', 'sandbox_create_folder', 'write_file', 'create_file', 'delete_file'];
                    if (fileOps.indexOf(obj.tool_name) !== -1 && typeof window.refreshTree === 'function') {
                      setTimeout(function() {
                        window.refreshTree();
                      }, 300);
                    }
