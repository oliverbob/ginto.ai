...
// Detect if this is an AJAX/API request that expects JSON
$isAjaxRequest = (
...
function createConfigurationFiles($data) {
    ...
    // Security Settings - CSRF is always enforced, no bypass options
...
function finalizeInstallationStep($data) {
    ...
    $configPath = '../config/llamacpp_models.json';
    file_put_contents($configPath, json_encode($llamaConfig, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
    $messages[] = "Model config saved: config/llamacpp_models.json";
    ...
    if (!empty($reasoningModel)) {
        ...
        $startScript .= "nohup llama-server -hf {$reasoningModel} -c 0 --host 0.0.0.0 --port 8034 > /tmp/llama-reasoning.log 2>&1 &\n";
    ...
    if (!empty($visionModel)) {
        ...
        $startScript .= "nohup llama-server -hf {$visionModel} --jinja -c 0 --host 0.0.0.0 --port 8033 > /tmp/llama-vision.log 2>&1 &\n";
    ...
    if ($hasModels) {
        ...
        $scriptPath = '../bin/start_llamacpp_models.sh';
        file_put_contents($scriptPath, $startScript);
        @chmod($scriptPath, 0755);
        $messages[] = "Start script created: bin/start_llamacpp_models.sh";
...
GINTO_DIR="{{GINTO_DIR}}"
CONFIG_FILE="${GINTO_DIR}/config/llamacpp_models.json"
LLAMA_SERVER=""
...
start_models() {
    ...
    if [[ -n "$reasoning_model" && "$reasoning_model" != "" ]]; then
        ...
        $LLAMA_SERVER -hf "$reasoning_model" -c 0 --host 0.0.0.0 --port "${reasoning_port:-8034}" &
    ...
    if [[ -n "$vision_model" && "$vision_model" != "" ]]; then
        ...
        $LLAMA_SERVER -hf "$vision_model" --jinja -c 0 --host 0.0.0.0 --port "${vision_port:-8033}" &
...
BOOTSTRAP;
        ...
        $bootstrapperPath = '../bin/llamacpp_bootstrapper.sh';
        file_put_contents($bootstrapperPath, $bootstrapper);
        @chmod($bootstrapperPath, 0755);
        $messages[] = "Bootstrapper created: bin/llamacpp_bootstrapper.sh";
...
ExecStart={$gintoDir}/bin/llamacpp_bootstrapper.sh start
ExecStop={$gintoDir}/bin/llamacpp_bootstrapper.sh stop
ExecReload={$gintoDir}/bin/llamacpp_bootstrapper.sh restart
Restart=on-failure
RestartSec=10
...
SYSTEMD;
        ...
        $servicePath = '../bin/ginto-llamacpp.service';
...
# Usage: sudo bash ~/ginto/bin/setup_llamacpp_boot.sh

set -e

GINTO_DIR="{{GINTO_DIR}}"
SERVICE_FILE="${GINTO_DIR}/bin/ginto-llamacpp.service"
...
if [[ $EUID -ne 0 ]]; then
   ...
   echo "   Usage: sudo bash ${GINTO_DIR}/bin/setup_llamacpp_boot.sh"
   exit 1
fi
...
cp "$SERVICE_FILE" /etc/systemd/system/ginto-llamacpp.service
...
# Enable service
echo "✅ Enabling ginto-llamacpp service..."
systemctl enable ginto-llamacpp

# Start service
echo "▶️  Starting ginto-llamacpp service..."
systemctl start ginto-llamacpp

# Check status
sleep 2
echo ""
echo "📊 Service status:"
systemctl status ginto-llamacpp --no-pager || true

echo ""
echo "✨ Done! llama.cpp models will now start automatically on boot."
echo ""
echo "📝 Useful commands:"
echo "   systemctl status ginto-llamacpp    # Check status"
echo "   systemctl restart ginto-llamacpp   # Restart models"
echo "   systemctl stop ginto-llamacpp      # Stop models"
echo "   journalctl -u ginto-llamacpp       # View logs"
echo ""
...
SETUP;
        ...
        $setupScriptPath = '../bin/setup_llamacpp_boot.sh';
        file_put_contents($setupScriptPath, $setupScript);
        @chmod($setupScriptPath, 0755);
        
        $messages[] = "Systemd service created: bin/ginto-llamacpp.service";
        $messages[] = "Setup script created: bin/setup_llamacpp_boot.sh";
        
        // Check if running in Docker container
        $isDocker = file_exists('/.dockerenv') || (getenv('INSTALL_MODE') === 'docker');
        
        // Auto-start the models immediately so they're available
        $startScriptPath = $gintoDir . '/bin/start_llamacpp_models.sh';
        if (file_exists($startScriptPath) && !$isDocker) {
            // Run the start script in background (native mode only)
            $output = [];
            $returnCode = 0;
            exec("bash " . escapeshellarg($startScriptPath) . " 2>&1", $output, $returnCode);
            
            if ($returnCode === 0) {
                $messages[] = "";
                $messages[] = "✅ llama.cpp models are starting in background...";
                $messages[] = "   Please wait 30-60 seconds for models to download and load.";
            } else {
                $messages[] = "";
        ...
        $messages[] = "   sudo bash ~/ginto/bin/setup_llamacpp_boot.sh";
        $messages[] = "";
        $messages[] = "💡 For boot persistence, run the setup_llamacpp_boot.sh script once.";
    }
