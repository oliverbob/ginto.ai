            ...
            if (this.postModalPostBtn) {
                this.postModalPostBtn.addEventListener('click', async () => {
                    ...
                    if (originalPostIdToShare) {
                        apiEndpoint = '/post/share';
                        payload.original_post_id = originalPostIdToShare;
                    } else {
                        apiEndpoint = '/post';
                    ...
                    try {
                        ...
                        if (response.ok && result.success && result.post) {
                            ...
                            if (response.status === 403) {
                                ...
                                alert(`Failed to ${originalPostIdToShare ? 'share' : 'create'} post: ${result.message || 'Unknown server error'}`);
                            }
                        }
                    } catch (error) {
