        ...
        <div class="wizard-container bg-white rounded-xl shadow-lg p-8 glow-card">
          ...
          <div class="wizard-content active" data-step="1">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <?php foreach ($tier_plans as $tier): ?>
                <?php
                  $isSelected = (isset($selected_tier) && $selected_tier['id'] == $tier['id']);
                  $cardClass = $isSelected ? 'selected border-2 border-indigo-300' : '';
                  $buttonClass = $isSelected ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-indigo-100 hover:bg-indigo-200 text-indigo-700';
                  $badgeText = '';
                  if ($tier['name'] === 'Starter') $badgeText = 'Basic';
                  if ($tier['name'] === 'Professional') $badgeText = 'Recommended';
                  if ($tier['name'] === 'Executive') $badgeText = 'Elite';
                ?>
                <div class="tier-card p-6 relative <?= $cardClass ?>" data-tier-id="<?= htmlspecialchars($tier['id']) ?>" data-tier-name="<?= htmlspecialchars($tier['name']) ?>" data-tier-price="<?= htmlspecialchars($tier['price']) ?>">
                  <?php if (!empty($badgeText)): ?>
                    <div class="tier-badge"><?= $badgeText ?></div>
                  <?php endif; ?>
                  <h4 class="text-xl font-bold text-gray-800 mb-2"><?= htmlspecialchars($tier['name']) ?></h4>
                  <p class="text-gray-600 mb-4"><?= htmlspecialchars($tier['description']) ?></p>
                  <div class="text-3xl font-extrabold text-indigo-600 mb-4">₱<?= number_format($tier['price'], 0) ?></div>
                  <ul class="space-y-2 mb-6 text-sm text-gray-700">
                    <li class="flex items-center"><i class="fas fa-check text-green-500 mr-2"></i> Access to Level 1-<?= htmlspecialchars($tier['commission_levels_access']) ?> commissions</li>
                    <?php if ($tier['name'] === 'Starter'): ?>
                    ...
                    <?php elseif ($tier['name'] === 'Professional'): ?>
                    ...
                    <?php elseif ($tier['name'] === 'Executive'): ?>
                  ...
                  <button type="button" class="w-full font-semibold py-2 px-4 rounded-lg transition-colors select-tier <?= $buttonClass ?>">
                    Select Plan
                  </button>
                </div>
              <?php endforeach; ?>
          ...
          <div class="wizard-content" data-step="3">
            ...
            <form id="tierPurchaseForm" action="/buytier/process" method="POST">
              ...
              <input type="hidden" name="tier_plan_id" id="hiddenTierPlanId" value="<?= htmlspecialchars($selected_tier['id'] ?? '') ?>">
