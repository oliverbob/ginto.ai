...
class WebhookController extends Controller
{
    ...
    private function handleSubscriptionActivated($subscription)
    {
        ...
        $start_date = isset($subscription['start_time']) ? date('Y-m-d H:i:s', strtotime($subscription['start_time'])) : null;
        $period_end_date = isset($subscription['billing_info']['next_billing_time']) ? date('Y-m-d H:i:s', strtotime($subscription['billing_info']['next_billing_time'])) : null;

        if (!$user_id || !$subscription_id || !$plan_id) {
            $this->logWebhook('CRITICAL: Missing user_id, subscription_id, or plan_id in ACTIVATED event.', $subscription);
            return;
        }

        $this->logWebhook('Subscription activated', [
        ...
        // Use Medoo to robustly insert or update the record.
        // This prevents errors if a record was already created by another process.
        if ($this->db->has('subscriptions', ['paypal_subscription_id' => $subscription_id])) {
            // If it exists, update it
            $this->db->update('subscriptions', $dataToStore, ['paypal_subscription_id' => $subscription_id]);
        } else {
            // If it's new, insert it
            $this->db->insert('subscriptions', $dataToStore);
        }
    }
    ...
    private function handleSubscriptionCancelled($subscription)
    {
        ...
        // When a subscription is cancelled, it usually remains active until the
        // end of the current billing period. We just update the status.
        $this->db->update('subscriptions', 
            ['status' => 'CANCELLED'], // Data to update
            ['paypal_subscription_id' => $subscription_id] // WHERE clause
        );
    }

    private function handleSubscriptionPaymentFailed($subscription)
    {
        $this->logWebhook('Subscription payment failed', ['subscription_id' => $subscription['id'] ?? '']);
        // Add your failed payment logic here
    }

    /**
     * Checks if the currently logged-in user has an active subscription to a specific plan.
     * This function queries the 'subscriptions' table.
     *
    ...
    public function isUserSubscribedToPlan(string $planId): bool
    {
        ...
        // Use Medoo to check your 'subscriptions' table
