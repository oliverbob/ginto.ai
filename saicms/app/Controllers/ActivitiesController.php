...
class ActivitiesController extends Controller {
    ...
    public function toggleLike() {
        ...
        try {
            if ($existingLike) {
                $this->db->delete("likes", ["id" => $existingLike['id']]);
                $isLiked = false;
            } else {
    ...
    public function addComment() {
        ...
        try {
            ...
            error_log("PDOException inserting comment: " . $e->getMessage());
            http_response_code(500); echo json_encode(['success' => false, 'error' => 'Database error saving comment.']); exit;
        }
        if (!$commentId || $commentId <= 0) {
            error_log("Failed insert comment (invalid ID: " . var_export($commentId, true) . "). DB Err: " . json_encode($this->db->error()));
    ...
    public function updatePost() {
        ...
        $updateData = ["content" => $content, "visibility" => $visibility, "updated_at" => Medoo::raw('NOW()')];
    ...
    public function deletePost($id) { // The $id parameter is passed by your router from the URL
        ...
        $postOwnerUserId = $this->db->get("posts", "user_id", ["id" => $postId]);

        if ($postOwnerUserId === false || $postOwnerUserId === null) {
            http_response_code(404);
        ...
        if ((int)$postOwnerUserId !== $this->currentUserId) {
        ...
        if ($deleteResult instanceof \PDOStatement && $deleteResult->rowCount() > 0) {
            ...
            $dbError = $this->db->error(); // Get database error info
            error_log("Failed to delete post {$postId}. User ID: {$this->currentUserId}. DB Error: " . ($dbError ? json_encode($dbError) : 'Unknown DB error or post not found for deletion criteria'));
            http_response_code(500);
